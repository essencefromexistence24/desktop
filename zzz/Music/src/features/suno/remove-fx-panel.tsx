...
import type {
  RemoveFxBusyState,
  RemoveFxJobView,
} from "@/features/ai/use-remove-fx-job";

type RemoveFxPanelProps = {
  busy: RemoveFxBusyState;
  job?: RemoveFxJobView;
  onRefresh: () => Promise<unknown>;
  onSave: () => Promise<unknown>;
};

export function RemoveFxPanel({
  busy,
  job,
  onRefresh,
  onSave,
      ...
      <CardHeader>
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <CardTitle className="flex items-center gap-2 text-base">
            <Eraser className="size-4 text-emerald-200" />
            Remove FX
          </CardTitle>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              title={actionGuard.title("Save clean audio")}
              onClick={() => {
                void onSave();
              }}
            >
      ...
      <CardContent className="space-y-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {job.id} / {job.cleanupTargets.join(", ")} / {job.intensity}
            {job.region
        ...
        {job.audio?.available && job.audio.audioUrl ? (
          ...
          <p className="text-sm text-muted-foreground">
            Waiting for the configured provider to return cleaned audio.
          </p>
        )}
      </CardContent>
    </Card>
