...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    if (!source) {
      return jsonError("Extended audio is not available for this job.", 404);
    }
    ...
    if (!isSafeRemoteAudioUrl(source.value)) {
      return jsonError("Extended audio URL is not supported.", 422);
    }
    ...
    try {
      ...
      if (!providerResponse.ok || !providerResponse.body) {
        return jsonError("Extension provider result could not be fetched.", 502);
      }
...
function dataUrlResponse(source: AudioSource) {
  ...
  if (!match) {
    return jsonError("Inline extension audio is malformed.", 422);
  }
...
function audioHeaders(mediaType: string) {
  return {
    ...
    "Content-Disposition": 'inline; filename="essence-extension"',
