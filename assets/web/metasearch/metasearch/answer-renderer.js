(() => {
  ...
  function renderAnswer({ payload, state, elements, apiOrigin }) {
    ...
    const user = renderMessage("user", cleanText(state.query || payload.query), { speaker, language });

    const summary = window.DxMetasearchAnswerSummary;
    const initialModel = summary ? "…" : "hardcode";
    const assistant = renderMessage("assistant", "", { assistant, latest, language, sourceText, modelName: initialModel });
