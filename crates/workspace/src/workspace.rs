...
impl Workspace {
    ...
    pub fn prompt_for_new_path(
        ...
        // Always prefer the registered (in-app) prompt for new path when available.
        // This ensures the save dialog for untitled files reliably shows (native system
        // save picker can fail to appear on some Windows/fork setups). Falls back to native only
        // if no custom prompt registered.
        if self.on_prompt_for_new_path.is_some()
