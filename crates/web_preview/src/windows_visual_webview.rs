    ...
    unsafe {
        if let Ok(environment10) = environment10 {
            ...
            if let Ok(options3) = options.cast::<ICoreWebView2ControllerOptions3>() {
                // Use an opaque white default background (matching a normal
                // browser) so pages without an explicit background, and the
                // brief pre-navigation/loading state, render as solid content
                // instead of a transparent surface that bleeds the compositor
                // through and looks broken.
