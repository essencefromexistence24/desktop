...
const DX_STYLE_RUNTIME_VALIDATION_RECEIPT_TEMPLATE_SCHEMA: &str =
    "zed.web_preview.dx_style.runtime_validation_receipt_template.v1";
const DX_STYLE_MUTATION_WRITE_RECEIPT_SCHEMA: &str =
...
const SOURCE_APPLY_MUTATION_WRITE_RECEIPT_FIELDS: &[&str] = &[
    "schema",
    "source_apply_receipt_schema",
...
pub(crate) fn source_apply_review_receipt(payload: &Value) -> Value {
    ...
    if !string_array_contains(
        ...
        reasons.push(
            "source-apply contract is missing runtime validation receipt template field"
                .to_string(),
        );
    }
    if !string_array_contains(
    ...
    let post_write_digest_verification_plan_status = post_write_digest_verification_plan
        .get("status")
        .and_then(Value::as_str);
    let runtime_validation_receipt_template = runtime_validation_receipt_template(
        editor_write_bridge,
        &native_writer_commit_plan,
        &post_write_digest_verification_plan,
    );
...
fn source_write_readiness(
    ...
    let runtime_validation_receipt_schema = editor_write_bridge
        ...
        .and_then(Value::as_str);
    let mutation_write_receipt_schema = editor_write_bridge
    ...
    let missing_required_mutation_write_receipt_fields =
        missing_required_mutation_write_receipt_fields(editor_write_bridge);
    ...
    let runtime_validation_receipt_template_ready =
        if contract_source_mutation_enabled == Some(true) {
            runtime_validation_receipt_template_status == Some("ready")
        } else {
            matches!(
                runtime_validation_receipt_template_status,
                Some("blocked_runtime_unverified" | "ready")
    ...
    if mutation_write_receipt_schema != Some(DX_STYLE_MUTATION_WRITE_RECEIPT_SCHEMA) {
        missing_requirements.push("write_bridge_mutation_write_receipt_schema_missing");
    }
    if !missing_required_mutation_write_receipt_fields.is_empty() {
        missing_requirements.push("write_bridge_required_mutation_write_receipt_fields_missing");
    }
    if !string_array_contains(
        editor_write_bridge,
    ...
    {
        missing_requirements.push("write_bridge_missing_webview_review_round_trip_runtime_proof");
    }
    if runtime_validation_required == Some(true)
        && !string_array_contains(
            editor_write_bridge,
    ...
    json!({
        ...
        "mutation_write_receipt_schema": mutation_write_receipt_schema,
        "required_mutation_write_receipt_fields": string_array_at(editor_write_bridge, "/required_mutation_write_receipt_fields"),
        "missing_required_mutation_write_receipt_fields": missing_required_mutation_write_receipt_fields,
...
fn missing_required_mutation_write_receipt_fields(editor_write_bridge: &Value) -> Vec<&str> {
    string_array_at(
        editor_write_bridge,
        "/required_mutation_write_receipt_fields",
    )
    .into_iter()
    .filter(|field| !SOURCE_APPLY_MUTATION_WRITE_RECEIPT_FIELDS.contains(field))
    .collect()
}
...
fn runtime_validation_receipt_template(
    editor_write_bridge: &Value,
    ...
    let required_runtime_validation_receipt_fields = string_array_at(
        editor_write_bridge,
        "/required_runtime_validation_receipt_fields",
    );
    let expected_source_digest_after = native_writer_commit_plan
        .get("expected_source_digest_after")
        .and_then(Value::as_str);
    let expected_post_write_readback_digest = post_write_digest_verification_plan
        .get("expected_source_digest_after")
        .and_then(Value::as_str);
    let template_inputs_ready = runtime_validation_receipt_schema
        == DX_STYLE_RUNTIME_VALIDATION_RECEIPT_SCHEMA
        && !required_runtime_validation_receipt_fields.is_empty()
        && expected_source_digest_after.is_some_and(is_source_digest)
        && expected_post_write_readback_digest == expected_source_digest_after;

    json!({
        "schema": DX_STYLE_RUNTIME_VALIDATION_RECEIPT_TEMPLATE_SCHEMA,
        "status": if template_inputs_ready {
            "blocked_runtime_unverified"
        } else {
            "blocked_template_incomplete"
        },
        "reason": if template_inputs_ready {
            "Runtime validation receipt template is ready for the authorized runtime proof window, but no mutation or readback has been performed."
        } else {
            "Runtime validation receipt template is missing schema, field, digest, or post-write readback expectations."
        },
        "runtime_validation_receipt_schema": runtime_validation_receipt_schema,
        "required_runtime_validation_receipt_fields": required_runtime_validation_receipt_fields,
        "source_apply_receipt_schema": DX_STYLE_SOURCE_APPLY_RECEIPT_SCHEMA,
        "source_path": native_writer_commit_plan
            .get("source_path")
            .and_then(Value::as_str),
        "source_digest_before": native_writer_commit_plan
            .get("source_digest_before")
            .and_then(Value::as_str),
        "source_digest_after": expected_source_digest_after,
        ...
        "native_writer_dry_run_replay": native_writer_commit_plan
            .get("status")
            .and_then(Value::as_str),
...
fn mutation_write_receipt_template(
    ...
    let mutation_write_receipt_schema = editor_write_bridge
        .get("mutation_write_receipt_schema")
        .and_then(Value::as_str)
        .unwrap_or(DX_STYLE_MUTATION_WRITE_RECEIPT_SCHEMA);
    let required_mutation_write_receipt_fields = string_array_at(
        editor_write_bridge,
        "/required_mutation_write_receipt_fields",
    );
    let expected_source_digest_after = native_writer_commit_plan
    ...
    let runtime_template_status = runtime_validation_receipt_template
        .get("status")
        .and_then(Value::as_str);
    let template_inputs_ready = mutation_write_receipt_schema
        == DX_STYLE_MUTATION_WRITE_RECEIPT_SCHEMA
        && !required_mutation_write_receipt_fields.is_empty()
        && expected_source_digest_after.is_some_and(is_source_digest)
        && matches!(
    ...
    json!({
        "schema": DX_STYLE_MUTATION_WRITE_RECEIPT_TEMPLATE_SCHEMA,
        "status": if template_inputs_ready {
        ...
        "reason": if template_inputs_ready {
            "Mutation write receipt template is ready for a future mutation-capable editor writer, but source mutation remains disabled."
        } else {
            "Mutation write receipt template is missing schema, field, digest, or runtime-template evidence."
        },
        "mutation_write_receipt_schema": mutation_write_receipt_schema,
        "required_mutation_write_receipt_fields": required_mutation_write_receipt_fields,
        "source_apply_receipt_schema": DX_STYLE_SOURCE_APPLY_RECEIPT_SCHEMA,
        "runtime_validation_receipt_schema": editor_write_bridge
            .get("runtime_validation_receipt_schema")
            .and_then(Value::as_str),
        "source_path": native_writer_commit_plan
            .get("source_path")
            .and_then(Value::as_str),
        "edit_span": native_writer_commit_plan.get("edit_span").cloned(),
        "replacement_text_bytes": native_writer_commit_plan
            .get("replacement_text_bytes")
            .and_then(Value::as_u64),
        "source_digest_before": native_writer_commit_plan
            .get("source_digest_before")
