...
fn main() {
    let _dx = dx_config::CodeDxConfig::load();
    let _ = std::fs::create_dir_all(&_dx.sr_dir);
    let _ = std::fs::create_dir_all(&_dx.receipts_dir);
...
#[derive(Parser, Debug)]
#[command(name = "dx", disable_version_flag = true, max_term_width = 100)] // rebranded binary
struct Args {
