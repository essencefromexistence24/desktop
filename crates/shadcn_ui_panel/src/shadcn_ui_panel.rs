...
impl ShadcnUiPanel {
    ...
    fn render_ui_history_row(
        ...
        let _copy_id = shadcn_element_id(id_prefix, &format!("copy-{id_suffix}"));
        let preview_id = shadcn_element_id(id_prefix, &format!("preview-{id_suffix}"));
        let _docs_id = shadcn_element_id(id_prefix, &format!("docs-{id_suffix}"));
        let _remove_id = shadcn_element_id(id_prefix, &format!("remove-{id_suffix}"));
        let _pin_id = shadcn_element_id(id_prefix, &format!("pin-{id_suffix}"));
        ...
        let can_insert = can_drag_into_editor(item.source);
        let _primary_action = if item.install_only {
            "Install"
        } else if can_insert {
            "Insert"
        } else {
        ...
        let _action_label = recent_ui_action_label(entry.action);
        let action_icon = recent_ui_action_icon(entry.action);
        ...
        ListItem::new(row_id)
            ...
            .end_slot(
                // Primary "Add" action shown by default (always visible), mirroring the
                // media panel rows which expose their main action without requiring hover.
                // Secondary actions (Preview, more menu) remain hover-revealed below.
                IconButton::new(primary_id, IconName::Plus)
                    .shape(ui::IconButtonShape::Square)
                    .style(ButtonStyle::Subtle)
                    .icon_size(IconSize::Small)
                    .tooltip(Tooltip::text(if can_insert { "Add UI Component" } else { "Open Documentation" }))
            ...
            .end_slot_on_hover(
                h_flex()
                    ...
                    .child(
                        PopoverMenu::new(format!("{}-more-menu", item.id))
                            .trigger(
                                IconButton::new(format!("{}-more-btn", item.id), IconName::Ellipsis)
                                    .shape(ui::IconButtonShape::Square)
                                    .style(ButtonStyle::Subtle)
                                    .icon_size(IconSize::Small)
                            )
                            ...
                            .menu(move |window, cx| {
                                ...
                                let _remove_item = remove_item.clone();
                                let _copy_action = copy_action;
                                ...
                                Some(ContextMenu::build(window, cx, move |menu, _window, _cx| {
                                            ...
                                            .handler({
                                                ...
                                                move |window, cx| {
                                                    ...
                                                    window.defer(cx, move |_window, cx| {
                                                        if let Some(panel) = panel_view.upgrade() {
                                    ...
                                    .item(
                                        ContextMenuEntry::new("Open Documentation")
...
fn recent_ui_action_icon(action: RecentUiAction) -> IconName {
    match action {
        RecentUiAction::Previewed => IconName::Eye,
        RecentUiAction::Copied => IconName::Copy,
        RecentUiAction::Inserted => IconName::Plus,
        RecentUiAction::Installed => IconName::Download,
        RecentUiAction::OpenedDocs => IconName::ArrowUpRight,
        RecentUiAction::Pinned => IconName::Pin,
    }
}
