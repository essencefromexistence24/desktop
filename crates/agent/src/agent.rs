...
impl LanguageModels {
    ...
    fn catalog_model_selection_error(&self, model_id: &AgentModelId) -> Option<String> {
        self.catalog_bridge
            .as_ref()?
            .catalog_selection_error(model_id.as_str())
    }

    fn resolved_model_id(&self, model_id: &AgentModelId) -> Option<AgentModelId> {
        ...
        let resolved = self.catalog_bridge.as_ref()?.resolve_model_id(
            model_id.as_str(),
            self.models.keys().map(|model_id| model_id.as_str()),
        )?;
        Some(AgentModelId::new(resolved))
    }
    ...
    fn map_language_model_to_info(
        ...
        acp_thread::AgentModelInfo {
            ...
            name: model.name().0,
            description: None,
            icon: Some(match model.icon().unwrap_or_else(|| provider.icon()) {
