...
#[derive(Clone)]
pub(crate) struct X11PreviewHost {
    window: Window,
    fixed: Fixed,
    parent_xid: Cell<u64>,
...
impl X11PreviewHost {
    pub(crate) fn new(
        ...
        window.realize();
        attach_transient_parent(&window, parent_xid)?;
        ...
        Ok(Self {
            window,
            fixed,
            parent_xid: Cell::new(parent_xid),
    ...
    pub(crate) fn set_parent_xid(&self, parent_xid: u64) -> Result<()> {
        if self.parent_xid.get() == parent_xid {
            return Ok(());
        }
        attach_transient_parent(&self.window, parent_xid)?;
        self.parent_xid.set(parent_xid);
        self.lower();
        Ok(())
    }
...
fn apply_window_bounds(window: &Window, fixed: &Fixed, bounds: Option<Bounds<Pixels>>) {
    let x = bounds
        .map(|bounds| f32::from(bounds.origin.x).round() as i32)
        .unwrap_or(0);
    let y = bounds
        .map(|bounds| f32::from(bounds.origin.y).round() as i32)
        .unwrap_or(0);
    let width = bounds
...
fn capture_host_window_image(window: &Window) -> Result<RgbaImage> {
    ...
    let pixbuf = gdk_window
        ...
        .ok_or_else(|| anyhow!("Failed to capture the Linux X11 web preview host window"))?;
...
fn pixbuf_to_rgba_image(pixbuf: &gdk_pixbuf::Pixbuf) -> Result<RgbaImage> {
    ...
    RgbaImage::from_vec(width as u32, height as u32, rgba)
        .ok_or_else(|| anyhow!("Failed to construct the Linux X11 screenshot image buffer"))
}
...
fn crop_to_webview_bounds(
    ...
    if x >= image_width || y >= image_height {
        return Err(anyhow!(
            "The Linux X11 web preview bounds are outside the captured host image"
        ));
    }
...
fn attach_transient_parent(window: &Window, parent_xid: u64) -> Result<()> {
    let gdk_window = window
        .window()
        .ok_or_else(|| anyhow!("The GTK X11 host window did not expose a native GDK window"))?;
