...
function externalConflictMessage(preflight: DesktopPreflightResult) {
  if (preflight.reason === "desktop_owned_backend_active") {
    return preflight.port
       ? `A desktop-owned Train server for this install is already running on port ${preflight.port}. Quit the other desktop app instance, then try again.`
      : "A desktop-owned Train server for this install is already running. Quit the other desktop app instance, then try again.";
  }

  if (preflight.reason === "desktop_owned_backend_starting") {
    return "The desktop-owned Train backend is still starting. Wait a moment, then try again.";
  }

  if (preflight.reason?.startsWith("desktop_owned_backend_unmanageable:")) {
    return preflight.port
      ? `A desktop-owned Train backend on port ${preflight.port} cannot be safely controlled by this desktop app. Stop that backend, then reopen Train.`
      : "A desktop-owned Train backend cannot be safely controlled by this desktop app. Stop that backend, then reopen Train.";
  }

  return preflight.port
    ? `A Train server for this install is already running from a terminal on port ${preflight.port}. Stop that server, or run \`train studio update\` from that terminal before using the desktop app.`
    : "A Train server for this install is already running from a terminal. Stop that server, or run `train studio update` from that terminal before using the desktop app.";
}
          ...
          if (preflight.can_auto_repair) {
            ...
            setBackendError(
              preflight.disposition === "owned_stale"
                ? "Desktop-owned Train backend is too old for this desktop app. Run `train studio update`, then restart Train."
                : "Managed Train install is too old. Run `train studio update`.",
            );
          }
          return;
  ...
  async function startManagedServer() {
    ...
    try {
      ...
      if (msg.includes("already running")) {
        startingRef.current = false;
        setBackendError(
          "Managed server is already running but did not report a port. Restart Train and try again.",
        );
        return;
      }
      setBackendError(msg);
    ...
    const onAuthFailed = (event: Event) => {
      const detail =
        event instanceof CustomEvent && typeof event.detail === "string"
          ? event.detail
          : "Desktop authentication failed. Update or repair the managed Train install, then restart Train.";
