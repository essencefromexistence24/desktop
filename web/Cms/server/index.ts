...
import { getSetupStatus, createSite } from './repositories/setup'
...
// Auto-setup: if no site or owner exists, create a default site and admin
// user so the app is immediately usable without going through the setup flow.
{
  const setup = await getSetupStatus(db)
  if (setup.needsSetup) {
    await createSite(db, 'my-site', {})
    await createUser(db, {
      id: nanoid(),
      email: 'admin@instatic.local',
      displayName: 'Admin',
      passwordHash: await hashPassword('admin12345678'),
      roleId: 'owner',
      allowOwnerRole: true,
    })
    console.log('[server] Auto-setup complete (default site + admin user created)')
  }
}
