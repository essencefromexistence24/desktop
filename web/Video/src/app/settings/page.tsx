...
export default async function SettingsPage() {
  ...
  const vercelLinked = existsSync(
    join(process.cwd(), ".vercel", "project.json"),
  );
  const deploymentUrlCaptured = Boolean(
    process.env.ESSENCE_RELEASE_DEPLOYMENT_URL,
  );
  const deploymentScreenshotCaptured = Boolean(
    process.env.ESSENCE_RELEASE_DEPLOYMENT_URL &&
    process.env.ESSENCE_RELEASE_SCREENSHOT_URL,
  );
  const desktopLaunchVerified =
    process.env.ESSENCE_DESKTOP_LAUNCH_VERIFIED === "1";
  const { isSignedIn, usageReview, generationReview } =
    await loadSettingsServerData({
      databaseConfigured,
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <Card className="shadow-none">
          ...
          <CardContent>
            <Alert>
              ...
              <AlertDescription>
                {textEditingConfigured
                  ? "Text editing actions are ready."
                  : "Text editing actions need setup."}{" "}
                {imageGenerationConfigured
                  ? "Image generation is ready."
                  : "Image generation needs setup before it can create assets."}
              </AlertDescription>
            </Alert>
          </CardContent>
        </Card>
          ...
          <CardHeader>
            <CardTitle className="flex items-center justify-between text-lg">
              ...
              <Badge variant="secondary">
                {usageReview
                  ? `${usageReview.dailyRemaining} left today`
                  : "Private"}
              </Badge>
            </CardTitle>
          </CardHeader>
            ...
            {usageReview ? (
              <>
                <div className="grid gap-3 sm:grid-cols-3">
                  <UsageMetric
                    label="Past hour"
                    value={`${usageReview.hourlyUsed}/${usageReview.limits.hourly}`}
                  />
                  <UsageMetric
                    label="Past day"
                    value={`${usageReview.dailyUsed}/${usageReview.limits.daily}`}
                  />
                  <UsageMetric
                    label="Recent success"
                    value={`${usageReview.summary.complete}/${usageReview.summary.total || 0}`}
                  />
                </div>
              ...
              <Alert>
                ...
                <AlertDescription>
                  Sign in to review recent AI activity, remaining limits, and
                  safe retry status.
                </AlertDescription>
              </Alert>
            )}
        ...
        <Card className="shadow-none">
            ...
            <CardTitle className="flex items-center justify-between text-lg">
              ...
              <Badge variant="secondary">
                {generationReview.length
                  ? `${generationReview.length} recent`
                  : "Private"}
              </Badge>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {generationReview.length ? (
              generationReview.map((record) => (
                <GenerationRecordRow
                  key={`${record.action}-${record.createdAt}-${record.outputAssetKind}`}
                  record={record}
                />
              ))
            ) : (
              <Alert>
                ...
                <AlertDescription>
                  Recent prompts, model usage, safety status, and generated
                  asset metadata will appear here after sign-in.
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
    ...
    <div className="rounded-md border border-border p-3 text-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="text-xs text-muted-foreground">
            {record.outputAssetKind === "none"
              ? "No saved asset"
              : `${record.outputAssetKind} output`}{" "}
            / {formatUsageDate(record.createdAt)}
          </div>
        </div>
        <div className="flex shrink-0 gap-2">
          <Badge
            variant={
              record.safetyStatus === "blocked"
                ? "destructive"
                : record.safetyStatus === "flagged"
                  ? "secondary"
                  : "outline"
            }
          >
      ...
      {record.outputSummary ? (
        <p className="mt-2 text-xs">{record.outputSummary}</p>
      ) : null}
