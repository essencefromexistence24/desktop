...
export default async function HostedReviewPage({
  params,
}: {
        ...
        <Card className="shadow-none">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              This hosted review link does not exist or has been removed.
            </p>
      ...
      <Card className="shadow-none">
        <CardHeader>
          <CardTitle className="flex flex-wrap items-center justify-between gap-3 text-lg">
            ...
            <Badge variant={unavailable ? "destructive" : "default"}>
              {unavailable ? "Unavailable" : "Review link"}
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-3">
            <ReviewMetric
              label="Access"
              value={permissionLabel(link.permission)}
            />
            <ReviewMetric label="Expires" value={formatDate(link.expiresAt)} />
            <ReviewMetric
              label="Export"
              value={link.exportName ?? "Project review"}
            />
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {unavailable || link.permission !== "download" ? (
              ...
              <Button asChild variant="outline">
                <a
                  href={metadataHref}
                  download={`${safeFilename(link.title)}-review.json`}
                >
                  <Download className="size-4" />
                  Download metadata
                </a>
