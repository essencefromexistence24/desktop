...
export async function GET(request: Request) {
  try {
    return apiJson(
      request,
      { ok: true, projects: await listSyncedProjects() },
      undefined,
      methods,
    );
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return apiJson(
      request,
      { ok: true, project: await upsertSyncedProject(payload) },
      undefined,
      methods,
    );
  } catch (error) {
...
function projectErrorResponse(request: Request, error: unknown) {
  if (error instanceof ProjectAuthError) {
    return apiJson(
      request,
      { ok: false, reason: "You must be signed in to sync projects." },
      { status: 401 },
      methods,
    );
  }
  ...
  if (error instanceof ProjectDataError) {
    return apiJson(
      request,
      { ok: false, reason: "Saved project data could not be loaded." },
      { status: 409 },
      methods,
    );
  }
  ...
  if (error instanceof ProjectSyncConflictError) {
    return apiJson(
      request,
      {
        ...
        reason:
          "Project changed in the signed-in library. Refresh or review versions before syncing again.",
  ...
  if (error instanceof ZodError) {
    return apiJson(
      request,
      { ok: false, reason: "Saved project data is invalid." },
      { status: 400 },
      methods,
    );
  }
  ...
  return apiJson(
    request,
    { ok: false, reason: "Project sync failed. Try again." },
    { status: 500 },
    methods,
  );
}
