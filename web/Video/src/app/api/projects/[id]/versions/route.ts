...
export async function GET(
  ...
  try {
    ...
    return apiJson(
      ...
      {
        ok: true,
        ...(await listSyncedProjectVersions(parseProjectIdParam(id))),
      },
      undefined,
      methods,
    );
...
export async function POST(
  ...
  try {
    ...
    return apiJson(
      ...
      {
        ok: true,
        project: await restoreSyncedProjectVersion(
          parseProjectIdParam(id),
          payload,
        ),
      },
      undefined,
...
function projectVersionErrorResponse(request: Request, error: unknown) {
  if (error instanceof ProjectAuthError) {
    return apiJson(
      request,
      { ok: false, reason: "You must be signed in to view project versions." },
      { status: 401 },
      methods,
    );
  }
  ...
  if (error instanceof ProjectAccessError) {
    return apiJson(
      request,
      { ok: false, reason: "You do not have access to this project." },
      { status: 403 },
      methods,
    );
  }
  ...
  if (error instanceof InvalidProjectIdError) {
    return apiJson(
      request,
      { ok: false, reason: "Project id is invalid." },
      { status: 400 },
      methods,
    );
  }
  ...
  if (error instanceof ProjectVersionNotFoundError) {
    return apiJson(
      request,
      { ok: false, reason: "Project version was not found." },
      { status: 404 },
      methods,
    );
  }
  ...
  if (error instanceof ProjectDataError) {
    return apiJson(
      request,
      { ok: false, reason: "Saved project version could not be loaded." },
      { status: 409 },
      methods,
    );
  }
  ...
  if (error instanceof ZodError) {
    return apiJson(
      request,
      { ok: false, reason: "Project version request is invalid." },
      { status: 400 },
      methods,
    );
  }
  ...
  return apiJson(
    request,
    { ok: false, reason: "Project version history failed. Try again." },
    { status: 500 },
    methods,
  );
}
