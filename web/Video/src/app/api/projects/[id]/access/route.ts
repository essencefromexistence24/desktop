...
export async function GET(
  ...
  try {
    ...
    return apiJson(
      ...
      {
        ok: true,
        access: await listServerWorkspaceAccess(parseProjectIdParam(id)),
      },
      undefined,
      methods,
    );
...
export async function POST(
  ...
  try {
    ...
    return apiJson(
      ...
      {
        ok: true,
        access: await mutateServerWorkspaceAccess(
...
function workspaceAccessErrorResponse(request: Request, error: unknown) {
  if (error instanceof ServerWorkspaceAccessAuthError) {
    return apiJson(
      ...
      {
        ok: false,
        reason: "You must be signed in to manage workspace access.",
      },
      { status: 401 },
      methods,
    );
  ...
  if (error instanceof ServerWorkspaceAccessForbiddenError) {
    return apiJson(
      ...
      {
        ok: false,
        reason: "You do not have permission to manage this workspace.",
      },
      { status: 403 },
      methods,
    );
  ...
  if (error instanceof ServerWorkspaceAccessNotFoundError) {
    return apiJson(
      request,
      { ok: false, reason: "Workspace access record was not found." },
      { status: 404 },
      methods,
    );
  }
  ...
  if (error instanceof ZodError) {
    return apiJson(
      request,
      { ok: false, reason: "Workspace access request is invalid." },
      { status: 400 },
      methods,
    );
  }
  ...
  return apiJson(
    request,
    { ok: false, reason: "Workspace access failed. Try again." },
    { status: 500 },
    methods,
  );
}
