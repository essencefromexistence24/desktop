  ...
  try {
    ...
    if (!userId) {
      return apiJson(
        ...
        {
          ok: false,
          reason: "You must be signed in to use advanced audio restoration.",
        },
        { status: 401 },
        methods,
      );
    ...
    if (!(file instanceof File)) {
      return apiJson(
        request,
        { ok: false, reason: "Upload an audio file for advanced restoration." },
        { status: 400 },
        methods,
      );
    }
    ...
    if (error instanceof ZodError) {
      return apiJson(
        request,
        { ok: false, reason: "Audio restoration response is invalid." },
        { status: 502 },
        methods,
      );
    }
    if (error instanceof AiPromptSafetyError) {
      return apiJson(
        request,
        { ok: false, reason: error.message, safetyStatus: error.review.status },
        { status: 400 },
        methods,
      );
    }
    ...
    return apiJson(
      request,
      { ok: false, reason: "Advanced audio restoration could not finish." },
      { status: 500 },
      methods,
    );
  }
