...
export async function POST(request: Request) {
  try {
    ...
    if (!userId) {
      return apiJson(
        request,
        { ok: false, reason: "You must be signed in to use AI editing." },
        { status: 401 },
        methods,
      );
    }
    ...
    if (error instanceof ZodError) {
      return apiJson(
        request,
        { ok: false, reason: "AI request is invalid." },
        { status: 400 },
        methods,
      );
    }
    ...
    return apiJson(
      request,
      {
        ok: false,
        reason:
          "Creative AI failed to finish the request. Try again with a shorter prompt.",
      },
      { status: 500 },
      methods,
    );
