...
export async function POST(request: Request) {
  try {
    ...
    if (!userId) {
      return apiJson(
        request,
        { ok: false, reason: "You must be signed in to generate captions." },
        { status: 401 },
        methods,
      );
    }
    ...
    if (!(file instanceof File)) {
      return apiJson(
        ...
        {
          ok: false,
          reason: "Upload an audio or video file to generate captions.",
        },
        { status: 400 },
        methods,
      );
    ...
    if (error instanceof ZodError) {
      return apiJson(
        request,
        { ok: false, reason: "Caption generation request is invalid." },
        { status: 400 },
        methods,
      );
    }
    ...
    return apiJson(
      request,
      {
        ok: false,
        reason:
          "Automatic captions could not finish. Try a shorter media file.",
      },
      { status: 500 },
      methods,
    );
