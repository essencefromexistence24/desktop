...
export async function POST(request: Request) {
  try {
    ...
    if (!userId) {
      return apiJson(
        request,
        { ok: false, reason: "You must be signed in to generate voiceovers." },
        { status: 401 },
        methods,
      );
    }
    ...
    if (error instanceof ZodError) {
      return apiJson(
        request,
        { ok: false, reason: "Voiceover generation request is invalid." },
        { status: 400 },
        methods,
      );
    }
    ...
    return apiJson(
      request,
      {
        ok: false,
        reason:
          "Voiceover generation could not finish. Try again or reduce the script length.",
      },
      { status: 500 },
      methods,
    );
