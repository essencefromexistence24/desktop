  ...
  try {
    ...
    if (!userId) {
      return apiJson(
        ...
        {
          ok: false,
          reason: "You must be signed in to save AI cleanup history.",
        },
        { status: 401 },
        methods,
      );
    ...
    if (error instanceof ZodError) {
      return apiJson(
        request,
        { ok: false, reason: "Audio cleanup history is invalid." },
        { status: 400 },
        methods,
      );
    }
    ...
    return apiJson(
      request,
      { ok: false, reason: "Audio cleanup history could not be saved." },
      { status: 500 },
      methods,
    );
  }
