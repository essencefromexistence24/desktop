  ...
  try {
    ...
    if (!userId) {
      return apiJson(
        ...
        {
          ok: false,
          reason: "You must be signed in to generate scene videos.",
        },
        { status: 401 },
        methods,
      );
    ...
    const result = await runAiSceneVideoGeneration(await request.json(), {
      userId,
    });
    ...
    if (error instanceof ZodError) {
      return apiJson(
        request,
        { ok: false, reason: "Scene video service response is invalid." },
        { status: 502 },
        methods,
      );
    }
    ...
    return apiJson(
      request,
      { ok: false, reason: "Scene video generation could not finish." },
      { status: 500 },
      methods,
    );
  }
