  ...
  try {
    ...
    if (!userId) {
      return apiJson(
        ...
        {
          ok: false,
          reason: "You must be signed in to use video enhancement.",
        },
        { status: 401 },
        methods,
      );
    ...
    if (!(file instanceof File)) {
      return apiJson(
        request,
        { ok: false, reason: "Upload a video file for enhancement." },
        { status: 400 },
        methods,
      );
    }
    ...
    if (error instanceof ZodError) {
      return apiJson(
        request,
        { ok: false, reason: "Video enhancement response is invalid." },
        { status: 502 },
        methods,
      );
    }
    ...
    return apiJson(
      request,
      { ok: false, reason: "Video enhancement could not finish." },
      { status: 500 },
      methods,
    );
  }
