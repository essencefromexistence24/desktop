...
export async function POST(
  ...
  try {
    ...
    return apiJson(
      request,
      { ok: true, invitation: await acceptServerWorkspaceInvitation(token) },
      undefined,
      methods,
    );
  } catch (error) {
...
function invitationAcceptErrorResponse(request: Request, error: unknown) {
  ...
  if (error instanceof ServerWorkspaceAccessForbiddenError) {
    return apiJson(
      ...
      {
        ok: false,
        reason: "This invitation belongs to a different email address.",
      },
      { status: 403 },
      methods,
    );
  ...
  if (error instanceof ServerWorkspaceInvitationNotFoundError) {
    return apiJson(
      request,
      { ok: false, reason: "Workspace invitation was not found." },
      { status: 404 },
      methods,
    );
  }
  ...
  if (error instanceof ServerWorkspaceInvitationUnavailableError) {
    return apiJson(
      ...
      {
        ok: false,
        reason: "This workspace invitation is no longer available.",
      },
      { status: 410 },
      methods,
    );
  ...
  return apiJson(
    request,
    { ok: false, reason: "Workspace invitation could not be accepted." },
    { status: 500 },
    methods,
  );
}
