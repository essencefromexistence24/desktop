...
export async function GET(
  ...
  try {
    ...
    return apiJson(
      request,
      { ok: true, comments: await listHostedReviewComments(token) },
      undefined,
      methods,
    );
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    return apiJson(
      request,
      { ok: true, comment: await createHostedReviewComment(token, payload) },
      undefined,
      methods,
    );
  } catch (error) {
...
function reviewCommentErrorResponse(request: Request, error: unknown) {
  ...
  if (error instanceof ZodError) {
    return apiJson(
      request,
      { ok: false, reason: "Hosted review comment is invalid." },
      { status: 400 },
      methods,
    );
  }
