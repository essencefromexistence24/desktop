...
export async function GET(request: Request) {
  try {
    return apiJson(
      request,
      { ok: true, links: await listHostedReviewLinks(requestOrigin(request)) },
      undefined,
      methods,
    );
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return apiJson(
      ...
      {
        ok: true,
        link: await createHostedReviewLink(payload, requestOrigin(request)),
      },
      undefined,
      methods,
    );
...
export async function PATCH(request: Request) {
  try {
    ...
    return apiJson(
      ...
      {
        ok: true,
        link: await updateHostedReviewLink(payload, requestOrigin(request)),
      },
      undefined,
      methods,
    );
...
function reviewLinkErrorResponse(request: Request, error: unknown) {
  ...
  if (error instanceof ZodError) {
    return apiJson(
      request,
      { ok: false, reason: "Hosted review link settings are invalid." },
      { status: 400 },
      methods,
    );
  }
