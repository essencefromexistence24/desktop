...
export async function GET(request: Request) {
  try {
    ...
    if (!asset) {
      return stockJson(
        request,
        { ok: false, reason: "Stock media could not be found." },
        404,
      );
    }
    ...
    if (asset.size > maxStockDownloadBytes) {
      return stockJson(
        ...
        {
          ok: false,
          reason: "Choose stock media under 25 MB for browser import.",
        },
        413,
      );
    }
    ...
    const upstream = await fetch(asset.sourceUrl, {
      headers: { "user-agent": "Essence Studio stock import" },
    });
    if (!upstream.ok || !upstream.body) {
      return stockJson(
        request,
        { ok: false, reason: "Stock media could not be downloaded." },
        502,
      );
    }
    ...
    return stockJson(
      request,
      { ok: false, reason: "Stock media import could not finish." },
      500,
    );
  }
}
