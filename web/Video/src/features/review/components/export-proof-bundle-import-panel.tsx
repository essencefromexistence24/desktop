...
import type {
  ExportProofBundleComparison,
  ExportProofBundleComparisonStatus,
      ...
      <CardContent className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <Input
            ...
            aria-label="Import proof bundle"
            onChange={onImport}
          />
        ...
        {importedAt ? (
          <div className="text-xs text-muted-foreground">
            Latest imported proof: {formatDate(importedAt)}
          </div>
        ) : null}
        {comparison ? (
          <div className="grid gap-2 lg:grid-cols-2">
            {comparison.items.slice(0, 8).map((item) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Badge variant={comparisonBadgeVariant(item.status)}>
            ...
            {comparison.items.length > 8 ? (
              <div className="rounded-md border border-dashed border-border p-3 text-xs text-muted-foreground">
                ...
                {comparison.items.length - 8} more comparison checks are
                available in the imported JSON and live bundle.
              </div>
            ) : null}
          </div>
        ) : (
          <div className="rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">
            Import an exported proof bundle to restore reviewer handoff evidence
            or compare it with this local review package.
          </div>
        )}
      </CardContent>
    </Card>
