...
import {
  cancelBrowserRender,
  exportProjectBundle,
  renderWithBrowserFfmpeg,
} from "@/lib/render/browser-renderer";
import { getLoadPhase, subscribe } from "@/lib/render/ffmpeg-loader";
import type {
  FfmpegLoadPhase,
  FfmpegLoadProgress,
} from "@/lib/render/ffmpeg-loader";
import {
  renderCompositeWithCanvas,
  renderCurrentFrameImage,
} from "@/lib/render/composite-renderer";
...
} from "@/lib/render/render-errors";
import { preflightRenderPlan } from "@/lib/render/render-preflight";
import {
  createExportQualityPreview,
  type ExportQualityPreview,
} from "@/lib/render/export-quality-preview";
import {
  createRenderHandoffPlan,
  type RenderHandoffReport,
} from "@/lib/render/render-handoff";
import {
...
export function ExportPanel() {
  ...
  const clearFinishedExportJobs = useEditorStore(
    (state) => state.clearFinishedExportJobs,
  );
  ...
  const [preflightWarningMessage, setPreflightWarningMessage] = useState<
    string | null
  >(null);
  const [batchMessage, setBatchMessage] = useState<string | null>(null);
  const [reviewMessage, setReviewMessage] = useState<string | null>(null);
  const [selectedBatchDeliveryIds, setSelectedBatchDeliveryIds] = useState<
    string[]
  >(["shorts-mp4", "thumbnail-png"]);
  const [conversionSettings, setConversionSettings] =
    useState<ExportConversionSettings>(DEFAULT_EXPORT_CONVERSION_SETTINGS);
  const isDesktopRuntime = useIsDesktopRuntime();
  const controllerRef = useRef<AbortController | null>(null);
  const batchCancelledRef = useRef(false);
  const [ffmpegLoadPhase, setFfmpegLoadPhase] =
    useState<FfmpegLoadPhase>(getLoadPhase());
  ...
  const preset =
    exportPresets.find((item) => item.id === presetId) ?? exportPresets[0];
  const selectedBatchDeliveries = socialDeliveryPresets.filter((item) =>
    selectedBatchDeliveryIds.includes(item.id),
  );
  const isExportBusy = isPreparingExport || isRendering || isBatchRendering;
  const renderPlanPreview = createRenderPlan(
    project,
    assets,
    preset.id,
    preset.format,
  ...
  const renderHandoff = createRenderHandoffPlan({
    project,
    assets,
    plan: renderPlanPreview,
    isDesktopRuntime,
  });
  ...
  function handlePresetChange(nextPresetId: string) {
    ...
    setConversionSettings((current) => ({
      ...current,
      backgroundMode: supportsTransparentBackgroundPreset(nextPreset)
        ? "transparent"
        : "project",
    }));
  }
  ...
  async function render(
    selectedPreset: ExportPreset = preset,
    options: {
      ignoreBusy?: boolean;
      conversionOverride?: Partial<ExportConversionSettings>;
      qaProject?: EditorProject;
    } = {},
  ) {
    ...
    const plan = createRenderPlan(
      ...
      { conversion: options.conversionOverride ?? conversionSettings },
    );
    const handoff = createRenderHandoffPlan({
      project: qaProject,
      assets,
      plan,
      isDesktopRuntime,
    ...
    try {
      ...
      const warnings =
        handoff.status === "desktop-recommended"
          ? [handoff.detail, ...preflight.warnings]
          : preflight.warnings;
      ...
      setPreflightMessage(
        "Export could not prepare. Check your media and try again.",
      );
      return false;
    } finally {
    ...
    const job = queueExport(selectedPreset.format, selectedPreset.id, {
      exportQaSnapshot: createExportQaSnapshot({
        project: qaProject,
        mediaAssets: assets,
        plan,
        handoff,
      }),
      mediaAttributionSummary: createMediaAttributionSummary({
        project: qaProject,
        mediaAssets: assets,
      }),
    });
    ...
    try {
      ...
      if (canUseNativeRenderAdapter(handoff)) {
        renderedFile = await renderWithNativeDesktopAdapter(
          createNativeRenderRequest({ job, project, plan, assets }),
          {
        ...
        renderedFile = await renderCurrentFrameImage(
          plan,
          project,
          assets,
          currentTime,
          {
            signal: controller.signal,
            onProgress: (progress) =>
              updateExportJob(job.id, { progress: Math.min(progress, 95) }),
          },
        );
      ...
      exportCaptionSidecarIfNeeded(
        plan.conversionOptions?.captionMode ?? "burn-in",
      );
      updateExportJob(job.id, {
        status: "complete",
        progress: 100,
      ...
      if (
        ...
        error instanceof NativeRenderCancelledError
      ) {
  ...
  async function renderBatch() {
    ...
    setBatchMessage(
      batchReadinessReport.reviewCount > 0
        ? `${batchReadinessReport.reviewCount} delivery ${batchReadinessReport.reviewCount === 1 ? "has" : "have"} review items visible in Batch QA.`
        : null,
    );
    ...
    try {
      for (const item of batchReadinessReport.items) {
        ...
        const ok = await render(item.preset, {
          ignoreBusy: true,
          conversionOverride: item.conversionSettings,
          qaProject: item.deliveryProject,
        });
        if (ok) completed += 1;
      }
    } finally {
      ...
      setBatchMessage(
        batchCancelledRef.current
          ? `${completed}/${total} delivery export${total === 1 ? "" : "s"} completed before cancellation.`
          : `${completed}/${total} delivery export${total === 1 ? "" : "s"} completed.`,
      );
  ...
  function toggleBatchDelivery(deliveryId: string) {
    setSelectedBatchDeliveryIds((current) =>
  ...
  function clearFinishedJobs() {
    ...
    if (cleared > 0) {
      setBatchMessage(
        `${cleared} finished export ${cleared === 1 ? "job" : "jobs"} cleared.`,
      );
    }
  }
  ...
  async function openExportReview(job: ExportJob) {
    ...
    try {
      const review = await createExportReviewPackage({
        job,
  ...
  function exportCaptionSidecarIfNeeded(captionMode: CaptionExportMode) {
    ...
    downloadTextFile(
      `${project.title || "project"}-captions.srt`,
      formatSrt(cues),
  ...
  return (
    <footer
      className="flex min-h-32 w-full max-w-full min-w-0 flex-col gap-2 overflow-hidden border-t border-border bg-card px-3 py-2"
      data-editor-region="export-workspace"
      tabIndex={-1}
    >
      <div className="flex w-full max-w-full min-w-0 flex-wrap items-center justify-between gap-x-4 gap-y-2 overflow-hidden">
        <div className="flex w-full min-w-0 flex-1 flex-wrap items-center gap-2 overflow-hidden">
          <Select value={presetId} onValueChange={handlePresetChange}>
            <SelectTrigger className="w-full min-w-0 max-w-[160px] sm:max-w-[220px] overflow-hidden">
          ...
          <ScrollArea className="w-full min-w-0 max-w-full sm:max-w-[420px]">
            <div className="flex items-center gap-1 pb-2">
              {socialDeliveryPresets.map((delivery) => (
                <Button
                  ...
                  size="sm"
                  variant={
                    selectedBatchDeliveryIds.includes(delivery.id)
                      ? "secondary"
                      : "outline"
                  }
              ...
              <Button
                ...
                disabled={isExportBusy || !batchReadinessReport.canStart}
              >
                Batch
              </Button>
          ...
          {ffmpegLoadPhase !== "ready" &&
          ffmpegLoadPhase !== "error" &&
          ffmpegLoadPhase !== "idle" ? (
          ...
          {exportJobs.slice(0, 3).map((job) => (
            <div key={job.id} className="w-full min-w-0 max-w-[16rem] sm:w-64">
              <div className="mb-1 flex justify-between gap-2 text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-1">
                  {job.exportQaSnapshot ? (
                    <Badge
                      variant={exportQaBadgeVariant(
                        job.exportQaSnapshot.status,
                      )}
                    >
              ...
              <div className="mb-1 flex items-center justify-between gap-2 text-[11px] text-muted-foreground">
                ...
                <span>
                  {job.renderedFile
                    ? formatBytes(job.renderedFile.size)
                    : job.format.toUpperCase()}
                </span>
              </div>
              {job.sourceSnapshot ? (
                <div className="mb-1 truncate text-[10px] text-muted-foreground">
                  v{formatExportVersion(job.sourceSnapshot.capturedAt)} ·{" "}
                  {job.sourceSnapshot.layerCount} layers ·{" "}
                  {job.sourceSnapshot.width}x{job.sourceSnapshot.height}
                </div>
              ) : null}
      ...
      <ConverterCompressorControls
        ...
        disabled={isExportBusy}
        onChange={(patch) =>
          setConversionSettings((current) => ({ ...current, ...patch }))
        }
      />
    </footer>
  );
...
function RenderRouteBadge({ report }: { report: RenderHandoffReport }) {
  const variant =
    report.status === "unsupported" || report.status === "desktop-required"
      ? "destructive"
      : report.status === "browser-ready" || report.status === "desktop-ready"
        ? "default"
        : "secondary";
  const routeHint =
    report.status === "desktop-required"
      ? "Desktop required"
  ...
  return (
    <div
      className="flex max-w-[260px] shrink-0 items-center gap-2 rounded-md border border-border px-2 py-1 text-xs text-muted-foreground"
      title={report.detail}
    >
    ...
    <ScrollArea className="rounded-md border border-border bg-background/70">
      <div className="flex min-w-0 items-center gap-2 px-2 py-1.5 pb-3">
        ...
        <Select
          ...
          disabled={disabled}
          onValueChange={(captionMode) =>
            onChange({ captionMode: captionMode as CaptionExportMode })
          }
        >
        ...
        <Select
          ...
          onValueChange={(backgroundMode) =>
            onChange({ backgroundMode: backgroundMode as CanvasBackgroundMode })
          }
        >
...
function ExportQualityPreviewBadge({
  preview,
}: {
  preview: ExportQualityPreview;
}) {
  if (!preview.applies) {
    return (
      <span className="shrink-0 text-[11px] text-muted-foreground">
  ...
  return (
    <div className="flex min-w-0 shrink-0 items-center gap-2 rounded-sm border border-border px-2 py-1 text-[11px] text-muted-foreground">
      <Badge
        variant={
          preview.label === "Risky compression" ? "destructive" : "outline"
        }
      >
        {preview.label}
      </Badge>
      ...
      {preview.warnings[0] ? (
        <span className="text-destructive">{preview.warnings[0]}</span>
      ) : null}
    </div>
  );
}
...
function projectSubtitleSidecarCues(
  layers: Array<{ kind: string; start: number; cues?: SubtitleCue[] }>,
) {
...
function exportJobPresetLabel(job: ExportJob) {
  return (
    exportPresets.find((preset) => preset.id === job.preset)?.label ??
    job.preset
  );
}

function exportQaBadgeVariant(
  status: NonNullable<ExportJob["exportQaSnapshot"]>["status"],
) {
