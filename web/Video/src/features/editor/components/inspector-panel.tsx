...
export function InspectorPanel() {
  ...
  const updateSelectedLayersBounds = useEditorStore(
    (state) => state.updateSelectedLayersBounds,
  );
  ...
  const isolateSelectedLayers = useEditorStore(
    (state) => state.isolateSelectedLayers,
  );
  const showAllLayers = useEditorStore((state) => state.showAllLayers);
  const setSelectedLayersHidden = useEditorStore(
    (state) => state.setSelectedLayersHidden,
  );
  const setSelectedLayersLocked = useEditorStore(
    (state) => state.setSelectedLayersLocked,
  );
  const groupSelectedLayers = useEditorStore(
    (state) => state.groupSelectedLayers,
  );
  const ungroupSelectedLayers = useEditorStore(
    (state) => state.ungroupSelectedLayers,
  );
  ...
  const addBlurredBackgroundForSelectedMediaLayers = useEditorStore(
    (state) => state.addBlurredBackgroundForSelectedMediaLayers,
  );
  const extractAudioFromSelectedVideoLayers = useEditorStore(
    (state) => state.extractAudioFromSelectedVideoLayers,
  );
  const replaceSelectedVideoAudio = useEditorStore(
    (state) => state.replaceSelectedVideoAudio,
  );
  const createFreezeFramesFromSelectedVideoLayers = useEditorStore(
    (state) => state.createFreezeFramesFromSelectedVideoLayers,
  );
  const layer = project.layers.find((item) => item.id === selectedLayerId);
  const selectedLayers = selectedLayerIds.length
    ? project.layers.filter((item) => selectedLayerIds.includes(item.id))
    : layer
      ? [layer]
      : [];
  const editableSelectionCount = selectedLayers.filter(
    (item) => !item.locked,
  ).length;
  ...
  const patchTransform = (patch: Partial<LayerTransform>) =>
    updateLayer(layer.id, { transform: { ...layer.transform, ...patch } });
  const patchStyle = (patch: Partial<LayerStyle>) =>
    updateLayer(layer.id, { style: { ...layer.style, ...patch } });
  const layerAsset = layer.assetId
    ? mediaAssets.find((asset) => asset.id === layer.assetId)
    : undefined;
  ...
  return (
    ...
    >
      <section
        className="min-w-0 space-y-5 overflow-x-hidden p-4"
        aria-label={`${layer.name} inspector`}
      >
        ...
        {layer.kind === "subtitle" ? (
          <InspectorCaptionSection layer={layer} />
        ) : null}
        <InspectorLayerReviewSection
          layer={layer}
