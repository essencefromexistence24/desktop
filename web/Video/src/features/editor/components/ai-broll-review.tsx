...
export function AiBrollReview({ output, onInsertBroll }: AiBrollReviewProps) {
  ...
  async function insertAcceptedBroll() {
    if (!onInsertBroll || isInserting || acceptedSuggestions.length === 0)
      return;
    ...
    try {
      ...
      setInsertMessage(
        "B-roll could not be inserted. Try fewer suggestions or a simpler search term.",
      );
    } finally {
    ...
    <div className="space-y-3">
      <div className="rounded-md border border-border bg-background p-3 text-sm">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            size="sm"
            onClick={() => void insertAcceptedBroll()}
            disabled={
              !onInsertBroll || isInserting || acceptedSuggestions.length === 0
            }
          >
        ...
        {insertMessage ? (
          <p className="mt-2 text-xs text-muted-foreground">{insertMessage}</p>
        ) : null}
      </div>
          ...
          >
            <div className="mb-2 flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                onClick={() => setAccepted(suggestion, index, true)}
              >
              ...
              <Button
                ...
                onClick={() => setAccepted(suggestion, index, false)}
              >
                <X className="size-4" />
                Skip
              </Button>
            ...
            <MessageResponse>
              {[suggestion.rationale, ...suggestion.searchNotes]
                .filter(Boolean)
                .join("\n\n")}
            </MessageResponse>
          </div>
...
function brollInsertMessage(summary: BrollInsertSummary) {
  if (summary.insertedCount === 0) {
    if (summary.skippedCount > 0)
      return "No matching stock media was found for the accepted B-roll suggestions.";
