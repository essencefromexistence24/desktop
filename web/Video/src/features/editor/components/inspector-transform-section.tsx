    ...
    <>
      <div className="grid grid-cols-2 gap-3">
        <NumberField
          ...
          min={1}
          onChange={(width) => onPatchTransform({ width })}
        />
        <NumberField
          label="Height"
          value={layer.transform.height}
          min={1}
          onChange={(height) => onPatchTransform({ height })}
        />
        <NumberField
          label="X"
          value={layer.transform.x}
          step={0.01}
          onChange={(x) => onPatchTransform({ x })}
        />
        <NumberField
          label="Y"
          value={layer.transform.y}
          step={0.01}
          onChange={(y) => onPatchTransform({ y })}
        />
      </div>
      <Field label="Rotation">
        <Slider
          ...
          onValueChange={([rotation]) =>
            onPatchTransform({ rotation: rotation ?? 0 })
          }
        />
      </Field>
      <div className="grid grid-cols-2 gap-2">
        <Button
          variant={layer.transform.flipX ? "secondary" : "outline"}
          onClick={() => onPatchTransform({ flipX: !layer.transform.flipX })}
        >
          Flip horizontal
        </Button>
        <Button
          variant={layer.transform.flipY ? "secondary" : "outline"}
          onClick={() => onPatchTransform({ flipY: !layer.transform.flipY })}
        >
          Flip vertical
        </Button>
      </div>
      <div className="grid grid-cols-3 gap-2">
        <Button
          variant="outline"
          onClick={onCenterSelectedLayers}
        ...
        <Button
          variant="outline"
          onClick={() => onFitSelectedLayersToCanvas("cover")}
          disabled={!editableSelectionCount}
        >
          Cover
        </Button>
        <Button
          variant="outline"
          onClick={() => onFitSelectedLayersToCanvas("contain")}
          disabled={!editableSelectionCount}
        >
          Contain
        </Button>
      ...
      {isMediaLayer ? (
        <FramingModeControls
          value={layer.transform.framing}
          onChange={(framing) => onPatchTransform({ framing })}
        />
      ) : null}
      {isMediaLayer ? (
        <CropControls
          ...
          onChange={(crop) => onPatchTransform({ crop })}
        />
      ) : null}
      {isMediaLayer ? (
        <MaskShapeControls
          layer={layer}
          onChange={(radius) => onPatchStyle({ radius })}
        />
      ) : null}
      {isMediaLayer ? (
        <Button
          variant="outline"
          onClick={onAddBlurredBackgroundForSelectedMediaLayers}
          disabled={!editableSelectionCount}
        >
...
function FramingModeControls({
  value = "fill",
  onChange,
}: {
  value?: LayerFramingMode;
  onChange: (value: LayerFramingMode) => void;
}) {
  return (
    <Field label="Framing">
      <div className="grid grid-cols-3 gap-2">
        {framingModes.map((mode) => (
          <Button
            key={mode.value}
            variant={value === mode.value ? "secondary" : "outline"}
...
function CropControls({
  ...
  const setCrop = (patch: Partial<NonNullable<LayerTransform["crop"]>>) =>
    onChange(normalizeLayerCrop({ ...crop, ...patch }));
  const sourceDimensions = {
    width: mediaAsset?.width ?? layer.transform.width,
    height: mediaAsset?.height ?? layer.transform.height,
  };
  const applyAspectPreset = (presetId: CropAspectPresetId) =>
    onChange(cropForAspectPreset(presetId, sourceDimensions));
  const applyFocusPreset = (presetId: CropFocusPresetId) =>
    onChange(alignCrop(crop, presetId));
...
function MaskShapeControls({
  ...
  const applyShape = (presetId: LayerMaskShapePresetId) =>
    onChange(maskShapeRadius(presetId, layer.transform));
        ...
        {layerMaskShapePresets.map((preset) => (
          <Button
            ...
            variant={
              ...
              Math.round(maskShapeRadius(preset.id, layer.transform))
                ? "secondary"
                : "outline"
            }
