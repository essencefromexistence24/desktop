...
export function StockAssetBrowser({
  isImporting,
  onImport,
}: StockAssetBrowserProps) {
  const [query, setQuery] = useState("city skyline");
  const [mediaType, setMediaType] = useState<StockMediaType>("image");
  const [activeAudioPreset, setActiveAudioPreset] =
    useState<StockAudioLibraryPreset | null>(null);
  ...
  async function searchStock(input?: {
    ...
    const nextAudioPreset =
      input?.audioPreset ??
      (nextMediaType === "audio" ? activeAudioPreset : null);
    ...
    try {
      ...
      const response = await fetch(
        clientApiUrl(`/api/stock/search?${params}`),
        { credentials: "include" },
      );
  ...
  return (
    <section className="min-w-0 space-y-2 overflow-hidden rounded-md border border-border p-2">
      ...
      <div className="grid min-w-0 grid-cols-[1fr_88px] gap-2 overflow-hidden">
        <Input
          value={query}
        ...
        <div className="grid grid-cols-2 gap-1">
          {stockAudioLibraryPresets.map((preset) => (
            <Button
              ...
              size="sm"
              variant={
                activeAudioPreset?.id === preset.id ? "secondary" : "ghost"
              }
      ...
      <Button
        ...
        variant="outline"
        className="w-full"
        onClick={() => void searchStock({ audioPreset: null })}
        disabled={isSearching || !query.trim() || !canUseOnlineActions}
      >
        {isSearching ? (
...
function StockAssetRow({
  ...
  return (
    <div className="grid w-full max-w-full grid-cols-[48px_minmax(0,1fr)_auto] items-center gap-2 overflow-hidden rounded-md border border-border bg-background p-2">
      <div className="grid h-10 w-12 place-items-center overflow-hidden rounded-sm bg-muted">
        {asset.thumbnailUrl ? (
      ...
      <div className="min-w-0">
        ...
        <div className="mt-1 flex flex-wrap gap-1">
          ...
          {asset.licenseLabel ? (
            <Badge variant="outline">{asset.licenseLabel}</Badge>
          ) : null}
        </div>
      </div>
      <div className="flex items-center gap-1">
        {asset.kind === "audio" ? (
          <Button
            ...
            disabled={disabled}
            onClick={() =>
              void onImport(asset, { addToTimeline: true, audioRole })
            }
          >
        ...
        <Button
          ...
          disabled={disabled}
          onClick={() => void onImport(asset, { audioRole })}
        >
