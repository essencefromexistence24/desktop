...
interface CaptionCueListEditorProps {
  ...
  onApplyCuts?: (
    proposals: TranscriptCutProposal[],
  ) => TranscriptCutApplySummary;
}
...
const emphasisOptions: Array<{
  value: NonNullable<SubtitleCue["emphasis"]>;
  label: string;
}> = [
        ...
        <div className="flex items-center justify-between gap-2 rounded-md border border-border p-2">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <Input
              ...
              value={shiftAmount}
              onChange={(event) =>
                setShiftAmount(safeShiftAmount(event.target.value))
              }
          ...
          <ButtonGroup>
            <TooltipButton
              label="Shift captions earlier"
              onClick={() =>
                onChange(shiftSubtitleCues(normalizedCues, -shiftAmount))
              }
            >
              <ArrowLeft className="size-3.5" />
            </TooltipButton>
            <TooltipButton
              label="Shift captions later"
              onClick={() =>
                onChange(shiftSubtitleCues(normalizedCues, shiftAmount))
              }
            >
            ...
            <TooltipButton
              label="Repair caption gaps and overlaps"
              onClick={() => onChange(repairSubtitleCueTiming(normalizedCues))}
            >
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <ButtonGroup>
                <TooltipButton
                  label="Split cue"
                  disabled={cue.end - cue.start <= 0.2}
                  onClick={() => splitCue(cue)}
                >
                  <Scissors className="size-3.5" />
                </TooltipButton>
                <TooltipButton
                  label="Merge with next cue"
                  disabled={index >= normalizedCues.length - 1}
                  onClick={() => mergeCueWithNext(index)}
                >
                ...
                <TooltipButton
                  label="Duplicate cue"
                  onClick={() =>
                    onChange(duplicateSubtitleCue(normalizedCues, cue.id))
                  }
                >
                ...
                <TooltipButton
                  label="Delete cue"
                  onClick={() =>
                    onChange(removeSubtitleCue(normalizedCues, cue.id))
                  }
                >
            ...
            <div className="grid grid-cols-2 gap-2">
              <TimeField
                ...
                value={cue.start}
                max={duration}
                onChange={(start) => updateCueCoalesced(cue.id, { start })}
              />
              <TimeField
                label="End"
                value={cue.end}
                max={duration}
                onChange={(end) => updateCueCoalesced(cue.id, { end })}
              />
            </div>
            <Textarea
              ...
              value={cue.text}
              onChange={(event) =>
                updateCueCoalesced(cue.id, { text: event.target.value })
              }
              aria-label={`Caption cue ${index + 1} text`}
            />
            <Select
              value={cue.emphasis ?? "normal"}
              onValueChange={(emphasis) =>
                updateCue(cue.id, {
                  emphasis: emphasis as SubtitleCue["emphasis"],
                })
              }
            >
