...
import {
  createObjectMask,
  normalizeLayerObjectMasks,
} from "@/lib/editor/object-masks";
          ...
          >
            <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
              ...
              <div className="flex items-center gap-1">
                <Button
                  size="sm"
                  variant={mask.enabled ? "secondary" : "outline"}
                  onClick={() =>
                    updateMask(mask.id, { enabled: !mask.enabled })
                  }
                >
                  {mask.enabled ? "On" : "Off"}
                </Button>
            ...
            <div className="grid grid-cols-2 gap-1">
              <Button
                size="sm"
                variant={mask.mode === "blur" ? "secondary" : "outline"}
                onClick={() => updateMask(mask.id, { mode: "blur" })}
              >
                Blur
              </Button>
              <Button
                size="sm"
                variant={mask.mode === "solid" ? "secondary" : "outline"}
                onClick={() => updateMask(mask.id, { mode: "solid" })}
              >
                Solid
              </Button>
            </div>
            <div className="grid grid-cols-4 gap-2">
              <PercentField
                label="X"
                value={mask.x}
                onChange={(x) => updateMask(mask.id, { x })}
              />
              <PercentField
                label="Y"
                value={mask.y}
                onChange={(y) => updateMask(mask.id, { y })}
              />
              <PercentField
                label="W"
                value={mask.width}
                onChange={(width) => updateMask(mask.id, { width })}
                min={1}
              />
              <PercentField
                label="H"
                value={mask.height}
                onChange={(height) => updateMask(mask.id, { height })}
                min={1}
              />
            </div>
            <div className="grid grid-cols-3 gap-2">
              ...
              <NumberField
                ...
                step={1}
                onChange={(opacity) =>
                  updateMask(mask.id, { opacity: opacity / 100 })
                }
              />
              <label className="space-y-1 text-[11px] text-muted-foreground">
                ...
                <Input
                  ...
                  value={mask.color}
                  onChange={(event) =>
                    updateMask(mask.id, { color: event.target.value })
                  }
                />
              </label>
            </div>
            <div className="grid grid-cols-2 gap-1">
              <Button
                size="sm"
                variant={mask.tracking === "none" ? "secondary" : "outline"}
                onClick={() => updateMask(mask.id, { tracking: "none" })}
              >
                Manual
              </Button>
              <Button
                size="sm"
                variant={mask.tracking === "center" ? "secondary" : "outline"}
                onClick={() => updateMask(mask.id, { tracking: "center" })}
              >
                Center track
              </Button>
            </div>
