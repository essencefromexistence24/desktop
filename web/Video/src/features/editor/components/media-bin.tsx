...
} from "@/features/editor/components/stock-asset-browser";
...
import {
  RecordingControls,
  type RecordingResultSettings,
} from "@/features/editor/components/recording-controls";
...
import {
  createMediaHealthReport,
  summarizeMissingMediaImpact,
...
export function MediaBin() {
  ...
  const favoriteMediaAssetIds = useEditorStore(
    (state) => state.favoriteMediaAssetIds,
  );
  const project = useEditorStore((state) => state.project);
  const addMediaAsset = useEditorStore((state) => state.addMediaAsset);
  const toggleFavoriteMediaAsset = useEditorStore(
    (state) => state.toggleFavoriteMediaAsset,
  );
  ...
  const [mediaMessage, setMediaMessage] = useState<MediaPanelMessage | null>(
    null,
  );
  ...
  const createMediaCollection = useEditorStore(
    (state) => state.createMediaCollection,
  );
  const removeMediaCollection = useEditorStore(
    (state) => state.removeMediaCollection,
  );
  const toggleMediaAssetCollection = useEditorStore(
    (state) => state.toggleMediaAssetCollection,
  );
  const restoreLastRemovedMedia = useEditorStore(
    (state) => state.restoreLastRemovedMedia,
  );
  ...
  const activeCollection = mediaCollections.find(
    (collection) => collection.id === activeCollectionId,
  );
  const recordedTakesCollection = mediaCollections.find(
    (collection) => collection.name.trim().toLowerCase() === "recorded takes",
  );
  const mediaHealth = useMemo(
    () => createMediaHealthReport(project, mediaAssets, favoriteMediaAssetIds),
    [project, favoriteMediaAssetIds, mediaAssets],
  );
  const mediaHealthByAssetId = useMemo(
    () => new Map(mediaHealth.assets.map((item) => [item.asset.id, item])),
    [mediaHealth],
  );
  const missingImpactSummary = useMemo(
    () => summarizeMissingMediaImpact(mediaHealth),
    [mediaHealth],
  );
  const missingAssetCount = mediaHealth.missingAssets;
  const missingIssueCount =
    mediaHealth.missingAssets + mediaHealth.missingReferenceCount;
  const favoriteMediaAssetIdsSet = useMemo(
    () => new Set(favoriteMediaAssetIds),
    [favoriteMediaAssetIds],
  );
  const favoriteAssetCount = mediaHealth.favoriteAssets;
  const unusedAssetCount = mediaHealth.unusedAssets;
  const activeCollectionAssetIds = useMemo(
    () => new Set(activeCollection?.assetIds ?? []),
    [activeCollection],
  );
  const recordedTakeAssets = useMemo(() => {
    ...
    return mediaAssets.filter(
      (asset) =>
        recordedTakeIds.has(asset.id) &&
        (asset.type === "video" || asset.type === "audio"),
    );
  }, [mediaAssets, recordedTakesCollection]);
  const uploadAsset = useMemo(
    () => mediaAssets.find((asset) => asset.id === uploadAssetId) ?? null,
    [mediaAssets, uploadAssetId],
  );
  const filteredMediaAssets = useMemo(
    () =>
      mediaAssets.filter((asset) => {
        ...
        if (mediaFilter === "favorites")
          return favoriteMediaAssetIdsSet.has(asset.id);
        if (mediaFilter === "collection")
          return (
            Boolean(activeCollection) && activeCollectionAssetIds.has(asset.id)
          );
        if (mediaFilter === "missing") return !asset.objectUrl;
        if (mediaFilter === "recoverable")
          return Boolean(assetHealth?.isRecoverable);
    ...
    [
      activeCollection,
      activeCollectionAssetIds,
      favoriteMediaAssetIdsSet,
      mediaAssets,
      mediaFilter,
  ...
  async function importDesktopFiles() {
    ...
    try {
      ...
      setMediaMessage({
        tone: "destructive",
        text: desktopImportFailureMessage(error),
      });
    } finally {
      setIsImporting(false);
    }
  ...
  async function importSelfHostedMedia(input: SelfHostedMediaImportInput) {
    ...
    try {
      ...
      setMediaMessage({
        tone: "default",
        text: `${asset.name} linked from your storage.`,
      });
      return true;
    } catch (error) {
      setMediaMessage({
        tone: "destructive",
        text: selfHostedMediaImportFailureMessage(error),
      });
      return false;
    } finally {
      setIsImporting(false);
  ...
  async function uploadToSelfHostedStorage(input: SelfHostedMediaUploadInput) {
    ...
    setMediaMessage(null);
    try {
      const uploadedAsset = await uploadMediaAssetToSelfHostedStorage(
        asset,
        input,
      );
      const verification = await verifySelfHostedUploadPublicUrl(
      ...
      setMediaMessage({
        tone: "destructive",
        text: selfHostedUploadFailureMessage(error),
      });
      return false;
    } finally {
      setIsImporting(false);
  ...
  async function reconnectMissingMedia(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setMediaMessage({ tone: "default", text: `${asset.name} reconnected.` });
    } catch {
      setMediaMessage({
        tone: "destructive",
        text: "Selected file could not reconnect this media item.",
      });
    } finally {
      setIsImporting(false);
  ...
  async function reconnectMissingMediaBatch(
    ...
    try {
      for (const asset of missingAssets) {
        const file = files.find(
          (f) => f.name.toLowerCase() === asset.name.toLowerCase(),
        );
        if (!file) continue;
        try {
          addMediaAsset(await reconnectBrowserMedia(asset, file));
      ...
      setMediaMessage({
        tone:
          failedCount > 0 || reconnectedCount === 0 ? "destructive" : "default",
        text:
          reconnectedCount > 0
            ? `${reconnectedCount} items reconnected.`
            : "No missing media matched.",
      });
    } finally {
      setIsImporting(false);
      event.target.value = "";
  ...
  function removeAsset(assetId: string) {
    ...
    setMediaMessage(
      result.removedAsset
        ? {
            tone: "default",
            text: mediaRemovalResultMessage(result.removedLayerCount),
          }
  ...
  function addCollection() {
    ...
    setMediaMessage({
      tone: "default",
      text: `${collection.name} collection created.`,
    });
  }
  ...
  function removeActiveCollection() {
    ...
    setMediaMessage({
      tone: "default",
      text: `${activeCollection.name} collection removed.`,
    });
  }
  ...
  function toggleAssetCollection(assetId: string) {
    ...
    setMediaMessage({
      tone: "default",
      text: added
        ? "Media added to this collection."
        : "Media removed from this collection.",
    });
  }

  function addRecordedMedia(
    asset: Parameters<typeof addMediaAsset>[0],
    mode: RecordingMode,
    ...
    if (!layerOptions) {
      setMediaMessage({
        tone: "default",
        text: `${asset.name} saved to recorded takes.`,
      });
      return;
    }
    ...
    setMediaMessage({
      ...
      text: layerId
        ...
        : "Recording saved, but could not be added to the timeline.",
    });
  }

  function ensureRecordedTakesCollection(assetId: string) {
    const collection =
      mediaCollections.find(
        (item) => item.name.trim().toLowerCase() === "recorded takes",
      ) ?? createMediaCollection("Recorded takes");
    ...
    const collection =
      mediaCollections.find(
        (item) =>
          ...
          stockAudioCollectionName.toLowerCase(),
      ) ?? createMediaCollection(stockAudioCollectionName);
    if (!collection || collection.assetIds.includes(assetId)) return;
    toggleMediaAssetCollection(collection.id, assetId);
  }

  function promoteRecordedTake(
    asset: Parameters<typeof addMediaAsset>[0],
    preset: RecordingResultSettings["timelinePreset"],
  ) {
    const layerOptions = recordingLayerOptions({
      asset,
    ...
    if (!layerOptions) {
      setMediaMessage({
        tone: "destructive",
        text: "Selected take could not be promoted.",
      });
      return;
    }
    const layerId = addLayerFromAsset(asset.id, layerOptions);
    if (layerId && !favoriteMediaAssetIdsSet.has(asset.id))
      toggleFavoriteMediaAsset(asset.id);
    setMediaMessage({
      tone: layerId ? "default" : "destructive",
      text: layerId
        ? `${asset.name} promoted to timeline.`
        : "Selected take could not be promoted.",
    });
  }
  ...
  async function importStockAsset(
    ...
    try {
      ...
      const response = await fetch(
        clientApiUrl(`/api/stock/download?${params}`),
        { credentials: "include" },
      );
      if (!response.ok) {
        setMediaMessage({
          tone: "destructive",
          text: await stockDownloadFailureMessage(response),
        });
        return;
      }
      const blob = await response.blob();
      const importedAsset = await saveBrowserMedia(
      ...
      const layerId =
        importedAsset.type === "audio" && options.addToTimeline
          ? addLayerFromAsset(importedAsset.id, {
              ...stockAudioLayerPatch(
                stockAudioRoleFromAsset(asset, options.audioRole),
              ),
              start: currentTime,
              track: nextRecordingTrack(project.layers),
              duration: Math.max(1, importedAsset.duration || 5),
              name: stockAudioLayerName(
                asset,
                stockAudioRoleFromAsset(asset, options.audioRole),
              ),
      ...
      setMediaMessage({
        tone:
          layerId === null && options.addToTimeline ? "destructive" : "default",
        text: stockImportResultMessage(asset, layerId, options),
      });
    } catch (error) {
      setMediaMessage({
        tone: "destructive",
        text: stockImportExceptionMessage(error),
      });
    } finally {
      setIsImporting(false);
    }
  ...
  return (
    <aside
      className="flex h-full min-h-0 w-full max-w-full min-w-0 flex-col overflow-hidden overflow-x-hidden border-r-0 bg-card/50"
      aria-label="Media library"
    >
      <ScrollArea
        className="h-full min-h-0 w-full max-w-full flex-1 overflow-hidden overflow-x-hidden"
        aria-label="Media library controls and assets"
      >
        <div className="flex min-h-0 w-full max-w-full min-w-0 flex-col gap-3 overflow-hidden overflow-x-hidden p-3">
          ...
          {mediaMessage ? (
            <div
              className={`w-full max-w-full min-w-0 overflow-hidden break-words rounded-md border p-2 text-xs ${mediaMessage.tone === "destructive" ? "border-destructive/40 text-destructive" : "border-border text-muted-foreground"}`}
            >
              {mediaMessage.text}
            </div>
          ) : null}
          {mediaMessage || lastRemovedMedia ? (
          ...
          <button
            ...
            className="flex w-full max-w-full min-w-0 items-center justify-between overflow-hidden rounded-md border border-border bg-background px-3 py-2 text-xs font-medium hover:bg-accent"
          >
            <span className="truncate">
              {showAdvancedTools ? "Hide advanced tools" : "More tools"}
            </span>
            ...
            <div className="flex w-full max-w-full min-w-0 flex-col gap-3 overflow-hidden">
              ...
              <MediaBinBatchActions
                ...
                onAssignFilteredToCollection={
                  assignFilteredAssetsToActiveCollection
                }
                onPrefixRenameFiltered={prefixRenameFilteredAssets}
                onRemoveUnused={removeUnusedAssets}
              />
              {/* Stock search disabled — route at _api (404) and needs clientApi runtime */}
              {/* <StockAssetBrowser isImporting={isImporting} onImport={importStockAsset} /> */}
              {/* Recording disabled — needs MediaRecorder + permissions, not core import flow */}
          ...
          >
            {mediaAssets.length === 0 ? (
              <div
                className="w-full max-w-full min-w-0 overflow-hidden break-words rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground"
                role="status"
              >
            ...
            {filteredMediaAssets.map((asset) => {
              const assetHealth = mediaHealthByAssetId.get(asset.id);
              const isFavorite =
                assetHealth?.isFavorite ??
                favoriteMediaAssetIdsSet.has(asset.id);
              const isInActiveCollection = activeCollectionAssetIds.has(
                asset.id,
              );
              return (
                <div
...
function selfHostedUploadResultMessage(
  assetName: string,
  verification: SelfHostedUploadVerificationResult,
) {
  if (verification.status === "verified")
    return `${assetName} uploaded and verified from your storage.`;
  if (verification.status === "failed")
    return `${assetName} uploaded, but the public URL check failed. ${verification.message}`;
  return `${assetName} uploaded. ${verification.message}`;
}

function reconnectResultMessage(
  assetName: string,
  remainingMissingCount: number,
) {
  if (remainingMissingCount === 0)
    return `${assetName} reconnected. All media is available.`;
...
function batchReconnectResultMessage(
  ...
  const parts = [
    `${reconnectedCount} ${reconnectedCount === 1 ? "item" : "items"} reconnected`,
  ];
...
function findReconnectFile(assetName: string, files: File[]) {
  ...
  return (
    files.find(
      (file) => normalizeMediaFilename(file.name) === normalizedAssetName,
    ) ??
    files.find(
      (file) =>
        mediaFilenameBase(normalizeMediaFilename(file.name)) ===
        normalizedAssetBase,
    )
  );
}
...
function stockImportResultMessage(
  asset: StockAsset,
  layerId: string | null,
  options: StockImportOptions,
) {
