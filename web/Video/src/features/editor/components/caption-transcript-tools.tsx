...
import {
  createTranscriptCutProposals,
  totalTranscriptCutDuration,
} from "@/lib/subtitles/transcript-cuts";
...
export function CaptionTranscriptTools({
  cues,
  onChange,
  onApplyCuts,
}: CaptionTranscriptToolsProps) {
  const [query, setQuery] = useState("");
  const [replacement, setReplacement] = useState("");
  const [showCutProposals, setShowCutProposals] = useState(false);
  const [applyMessage, setApplyMessage] = useState<string | null>(null);
  const matchCount = useMemo(
    () => countSubtitleTranscriptMatches(cues, query),
    [cues, query],
  );
  const cutProposals = useMemo(
    () => createTranscriptCutProposals(cues, { query }),
    [cues, query],
  );
  const proposedCutDuration = useMemo(
    () => totalTranscriptCutDuration(cutProposals),
    [cutProposals],
  );
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <Input
              ...
              placeholder="Find transcript text"
            />
          </div>
          <Badge variant={matchCount > 0 ? "secondary" : "outline"}>
            {matchCount}
          </Badge>
        </div>
        <div className="flex items-center gap-2">
          ...
          <ButtonGroup>
            <TooltipButton
              ...
              onClick={() =>
                onChange(
                  replaceSubtitleTranscriptText(cues, query, replacement),
                )
              }
            >
            ...
            <TooltipButton
              label="Clean transcript casing"
              disabled={cues.length === 0}
              onClick={() => onChange(cleanSubtitleTranscriptCasing(cues))}
            >
        ...
        {showCutProposals && cutProposals.length > 0 ? (
          <div className="space-y-1 rounded-md border border-border bg-background p-2">
            <div className="flex items-center justify-between text-[11px] text-muted-foreground">
              <span>
                {cutProposals.length} proposed cut
                {cutProposals.length === 1 ? "" : "s"}
              </span>
            ...
            <Button
              ...
              size="sm"
              variant="outline"
              disabled={!onApplyCuts}
              onClick={applyTranscriptCuts}
            >
              <Scissors className="size-3.5" />
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge variant="outline">
                    {formatTime(proposal.duration)}
                  </Badge>
                </div>
                <div className="mt-1 text-[11px] text-muted-foreground">
                  {proposal.cueIds.length} caption
                  {proposal.cueIds.length === 1 ? "" : "s"}
                </div>
...
function transcriptCutApplyMessage(summary: TranscriptCutApplySummary) {
  ...
  if (summary.changedLayerCount === 0)
    return `${summary.rangeCount} transcript cut${summary.rangeCount === 1 ? "" : "s"} reviewed; no editable layers crossed those ranges.`;
