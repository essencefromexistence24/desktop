...
export function VisualEffectsPanel({
  ...
  const backgroundReplacement = normalizeLayerBackgroundReplacement(
    visualStyle.backgroundReplacement,
  );
  ...
  return (
    <div className="space-y-3 rounded-md border border-border p-2">
      ...
      <EffectSlider
        label="Brightness"
        value={visualStyle.brightness ?? 1}
        min={0}
        max={2}
        step={0.01}
      ...
      <EffectSlider
        label="Saturation"
        value={visualStyle.saturation ?? 1}
        min={0}
        max={2}
        step={0.01}
        ...
        <div className="space-y-2 rounded-md border border-border p-2">
          <label className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
            ...
            <input
              ...
              onChange={(event) =>
                onChange({
                  chromaKey: { ...chromaKey, enabled: event.target.checked },
                })
              }
            />
          </label>
          {chromaKey.enabled ? (
            <>
              <ColorField
                ...
                onChange={(color) =>
                  onChange({ chromaKey: { ...chromaKey, color } })
                }
              />
              <EffectSlider
                label="Similarity"
                ...
                suffix=""
                onChange={(similarity) =>
                  onChange({ chromaKey: { ...chromaKey, similarity } })
                }
              />
              <EffectSlider
                label="Smoothness"
                ...
                suffix=""
                onChange={(smoothness) =>
                  onChange({ chromaKey: { ...chromaKey, smoothness } })
                }
              />
              <EffectSlider
                label="Spill"
                ...
                suffix=""
                onChange={(spill) =>
                  onChange({ chromaKey: { ...chromaKey, spill } })
                }
              />
              <label className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
      ...
      <div className="grid grid-cols-2 gap-2">
        <ColorField
          label="Border"
          value={visualStyle.stroke}
          onChange={(stroke) => onChange({ stroke })}
        />
        <EffectNumber
          label="Border width"
          value={visualStyle.borderWidth ?? 0}
          min={0}
          max={80}
        ...
        <ColorField
          label="Shadow"
          value={visualStyle.shadowColor ?? "#000000"}
          onChange={(shadowColor) => onChange({ shadowColor })}
        />
        <EffectNumber
          label="Shadow blur"
          value={visualStyle.shadowBlur ?? 0}
          min={0}
          max={120}
