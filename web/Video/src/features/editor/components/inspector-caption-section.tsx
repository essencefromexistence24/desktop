...
export function InspectorCaptionSection({ layer }: { layer: TimelineLayer }) {
  ...
  const addSubtitleLayerFromCues = useEditorStore(
    (state) => state.addSubtitleLayerFromCues,
  );
  ...
  const [pendingCaptionImport, setPendingCaptionImport] = useState<{
    filename: string;
    cues: SubtitleCue[];
  } | null>(null);
  const text = (layer.cues ?? [])
  ...
  function applyCues(cues: SubtitleCue[], options?: { history?: boolean }) {
    ...
    const patch = {
      ...
      duration: Math.max(
        layer.duration,
        ...normalizedCues.map((cue) => cue.end),
        5,
      ),
    };
  ...
  async function importCaptions(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setCaptionMessage(
        `${cues.length} caption${cues.length === 1 ? "" : "s"} ready to import.`,
      );
    } catch {
  ...
  function replaceImportedCaptions(cues: SubtitleCue[]) {
    ...
    setCaptionMessage(
      `${cues.length} caption${cues.length === 1 ? "" : "s"} replaced.`,
    );
  }
  ...
  function mergeImportedCaptions(cues: SubtitleCue[]) {
    ...
    setPendingCaptionImport(null);
    setCaptionMessage(
      `${cues.length} caption${cues.length === 1 ? "" : "s"} merged.`,
    );
  }

  function createCaptionLayerFromImport(filename: string, cues: SubtitleCue[]) {
    const layerId = addSubtitleLayerFromCues({
      name: filename.replace(/\.[^.]+$/, "") || "Imported captions",
      cues,
    });
    setPendingCaptionImport(null);
    setCaptionMessage(
      layerId
        ? "Caption layer created."
        : "Caption layer could not be created.",
    );
  }
  ...
  function applyTranscriptCuts(proposals: TranscriptCutProposal[]) {
    return applyTimelineCutRanges(
      proposals.map((proposal) => ({
        start: Math.max(
          layer.start,
          layer.start + proposal.start - layer.trimStart,
        ),
        end: Math.min(
          layer.start + layer.duration,
          layer.start + proposal.end - layer.trimStart,
        ),
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex gap-1">
          ...
          <Button
            ...
            onClick={() =>
              exportCaptions(
                `${layer.name}-transcript.txt`,
                formatTranscript(layer.cues ?? []),
                "text/plain",
              )
            }
          >
      ...
      {pendingCaptionImport ? (
        <CaptionImportPreview
          ...
          onNewLayer={() =>
            createCaptionLayerFromImport(
              pendingCaptionImport.filename,
              pendingCaptionImport.cues,
            )
          }
          onCancel={() => setPendingCaptionImport(null)}
        />
      ) : null}
      <CaptionCueListEditor
        cues={layer.cues ?? []}
        duration={Math.max(layer.duration, 5)}
        onChange={applyCues}
        onApplyCuts={applyTranscriptCuts}
      />
      <Textarea
        value={text}
        onChange={(event) =>
          applyCues(parseManualCaptionText(event.target.value, layer.cues), {
            history: false,
          })
        }
...
function parseManualCaptionText(
  value: string,
  ...
  return normalizeSubtitleCues(
    value
      .split("\n")
      .map((line, index) =>
        parseManualCaptionLine(line, index, existingCues[index]?.id),
      ),
  );
}

function parseManualCaptionLine(
  line: string,
  index: number,
  existingId?: string,
): SubtitleCue {
...
function createManualCaptionCue(
