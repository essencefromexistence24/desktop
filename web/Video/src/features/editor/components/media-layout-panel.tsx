...
import {
  findImageToolPreset,
  imageToolPresets,
} from "@/lib/editor/image-tool-presets";
...
export function MediaLayoutPanel() {
  ...
  const selectedExportPreset = exportPresets.find(
    (preset) => preset.id === selectedImageTool.exportPresetId,
  );
  const layoutAssets = mediaAssets.filter(
  ...
  const effectiveAssets = selectedIds.length
    ...
    : layoutAssets.slice(
        0,
        Math.min(selectedImageTool.assetLimit, limitForMode(mode)),
      );
  ...
  function createLayout() {
    ...
    addMediaLayout({
      assetIds: effectiveAssets
        .slice(0, selectedImageTool.assetLimit)
        .map((asset) => asset.id),
      mode,
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-md border border-border p-2 text-xs text-muted-foreground">
        {selectedImageTool.aspectRatio} /{" "}
        {selectedExportPreset?.label ?? selectedImageTool.exportPresetId}
      </div>
      <div className="grid grid-cols-[1fr_82px] gap-2">
        <div className="space-y-1.5">
          ...
          <Select
            value={mode}
            onValueChange={(value) => setMode(value as MediaLayoutMode)}
          >
          ...
          <div className="space-y-1">
            {layoutAssets.slice(0, 12).map((asset) => {
              const selected =
                ...
                (!selectedIds.length &&
                  effectiveAssets.some((item) => item.id === asset.id));
              return (
                <button
                  key={asset.id}
      ...
      <Button
        ...
        disabled={!effectiveAssets.length}
      >
