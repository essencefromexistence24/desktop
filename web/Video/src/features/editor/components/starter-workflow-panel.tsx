...
import {
  findStarterWorkflowPreset,
  starterWorkflowPresets,
} from "@/lib/editor/starter-workflows";

export function StarterWorkflowPanel() {
  const [workflowId, setWorkflowId] = useState(
    starterWorkflowPresets[0]?.id ?? "",
  );
  ...
  const selectedSocialFormat = findSocialFormatPreset(
    selectedWorkflow.socialFormatId,
  );
  ...
  function startWorkflow() {
    loadProject(
      createSocialFormatProjectResize(project, selectedSocialFormat),
      mediaAssets,
    );
    selectedWorkflow.templateIds.forEach((templateId) =>
      addTemplate(templateId),
    );
    queueExport(selectedExportPreset.format, selectedExportPreset.id);
    setMessage(
      `${selectedWorkflow.label} started on ${selectedSocialFormat.label}; ${selectedExportPreset.label} queued.`,
    );
  }
          ...
          >
            <span className="flex items-center justify-between gap-2">
              ...
              <Badge variant="secondary">
                {workflow.templateIds.length} templates
              </Badge>
            </span>
