...
export function InspectorLayerReviewSection({
  layer,
  onUpdateLayer,
}: InspectorLayerReviewSectionProps) {
  ...
  return (
    <div className="space-y-3">
      <Field label="Review status">
        <Select
          value={layer.reviewStatus ?? "none"}
          onValueChange={(value) =>
            updateReviewStatus(value as LayerReviewStatus)
          }
        >
          <SelectTrigger>
            <SelectValue />
      ...
      <Field label="Layer notes">
        <Textarea
          value={layer.notes ?? ""}
          onChange={(event) =>
            onUpdateLayer(layer.id, { notes: event.target.value })
          }
          placeholder="Add edit notes or handoff context"
        />
      </Field>
      {layer.reviewUpdatedAt ? (
        <p className="text-[11px] text-muted-foreground">
          Review updated {formatReviewDate(layer.reviewUpdatedAt)}
        </p>
      ) : null}
    </div>
  );
