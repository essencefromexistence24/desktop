...
import {
  createSavedTimelineTemplate,
  editorTemplates,
  instantiateSavedTemplate,
...
type EditorTemplateSlice = Pick<
  EditorState,
  | "addTemplate"
  | "addSavedTemplate"
  | "saveCurrentTimelineAsTemplate"
  | "removeSavedTemplate"
>;
...
export function createEditorTemplateSlice(
  set: EditorStoreSet,
  get: EditorStoreGet,
  deps: EditorTemplateDeps,
): EditorTemplateSlice {
  return {
    addTemplate: (templateId) => {
      ...
      const layers = template.createLayers(
        ...
        {
          brandColors: brandColorsForTemplate(
            get().brandColors,
            get().project.brandKit,
          ),
        },
      );
    ...
    addSavedTemplate: (templateId) => {
      const template = get().savedTemplates.find(
        (item) => item.id === templateId,
      );
      if (!template) return { addedLayerCount: 0, missingAssetCount: 0 };

      const availableAssetIds = new Set(
        get().mediaAssets.map((asset) => asset.id),
      );
      const result = instantiateSavedTemplate(
        template,
        deps.nextTrack(get().project.layers),
        availableAssetIds,
      );
      ...
      return {
        addedLayerCount: result.layers.length,
        missingAssetCount: result.missingAssetCount,
      };
    },
    saveCurrentTimelineAsTemplate: (label) => {
      const template = createSavedTimelineTemplate(
        label ?? `${get().project.title} template`,
        get().project.layers,
      );
