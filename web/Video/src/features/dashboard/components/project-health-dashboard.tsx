    ...
    <div className={`grid gap-2 sm:grid-cols-4 ${className}`}>
      ...
      <div className="rounded-md border border-border p-3 text-sm">
        ...
        <div className="mt-1 flex flex-wrap gap-1">
          ...
          <Badge
            variant={
              report.reconnectRequiredMedia > 0 ? "destructive" : "outline"
            }
          >
            {report.reconnectRequiredMedia} reconnect
          </Badge>
          <Badge variant={report.reviewItems > 0 ? "default" : "outline"}>
            {report.reviewItems} review
          </Badge>
        </div>
      </div>
    </div>
...
export function matchesHealthFilter(
  summary: ProjectHealthSummary | undefined,
  filter: HealthFilter,
) {
...
function HealthMetric({
  ...
  return (
    <div className="flex items-center justify-between rounded-md border border-border p-3">
      ...
      <Icon
        className={`size-4 ${status === "blocked" ? "text-destructive" : status === "attention" ? "text-amber-300" : "text-primary"}`}
      />
    </div>
  );
}
