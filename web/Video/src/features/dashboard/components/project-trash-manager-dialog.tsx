...
export function ProjectTrashManagerDialog({
  ...
  const [activeAction, setActiveAction] = useState<"restore" | "delete" | null>(
    null,
  );
  ...
  const allSelected =
    projects.length > 0 &&
    projects.every((project) => selectedIdSet.has(project.id));
  ...
  async function runTrashAction(action: "restore" | "delete", ids: string[]) {
    ...
    try {
      const succeeded =
        action === "restore"
          ? await onRestoreProjects(ids)
          : await onDeleteProjects(ids);
      if (succeeded) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Restore recoverable projects or permanently remove old local
            records.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border bg-muted/30 p-2">
            <div className="flex items-center gap-2 text-sm">
              <input
                ...
                disabled={
                  isPending || activeAction !== null || projects.length === 0
                }
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                disabled={
                  isPending || activeAction !== null || selectedIds.length === 0
                }
              >
          ...
          <div className="max-h-96 space-y-2 overflow-y-auto pr-1">
            {projects.length === 0 ? (
              ...
              projects.map((project) => (
                <div
                  key={project.id}
                  className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-md border border-border p-3"
                >
                  <input
                    type="checkbox"
                    className="size-4 rounded border-border"
