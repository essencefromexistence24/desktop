    ...
    <div className="rounded-md border border-border p-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground">
            {items.length > 0
              ? `${items.length} guided ${items.length === 1 ? "recovery" : "recoveries"} ready`
              : "No local recovery actions are pending"}
          </div>
        </div>
        <Badge variant={items.length > 0 ? "secondary" : "default"}>
          {items.length}
        </Badge>
      </div>
      <div className="mt-3 grid gap-2">
        {items.length > 0 ? (
          ...
          <div className="flex items-center gap-2 rounded-md bg-muted/40 p-2 text-xs text-muted-foreground">
            ...
            Local proof, media, export queue, and sync handoff do not need
            recovery.
          </div>
        )}
      </div>
      <div className="mt-3 rounded-md bg-background/60 p-2">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Badge
            variant={completion.status === "resolved" ? "default" : "secondary"}
          >
            {completion.resolvedCount} resolved / {completion.pendingCount}{" "}
            pending
          </Badge>
        </div>
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          {completion.items.map((item) => (
            <div key={item.id} className="rounded-md border border-border p-2">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="text-xs font-medium">{item.label}</span>
                <Badge variant={completionBadgeVariant(item.status)}>
                  {localMaintenanceRecoveryCompletionLabel(item.status)}
                </Badge>
              </div>
...
function completionBadgeVariant(
...
function recoveryAction(
  ...
  if (item.id === "retry-exports") {
    return (
      <Button
        ...
        size="sm"
        variant="outline"
        onClick={actions.onRetryFailedExports}
      >
  ...
  if (item.id === "review-cloud-conflicts") {
    return (
      <Button
        ...
        size="sm"
        variant="outline"
        onClick={actions.onClearCloudConflicts}
      >
        {item.actionLabel}
      </Button>
    );
  ...
  return (
    <Button
      ...
      size="sm"
      variant="outline"
      onClick={actions.onRefreshProof}
    >
