...
import {
  importDesktopVerificationHistoryEntry,
...
import {
  createReleasePacketImportConflictPreview,
  type ReleasePacketImportConflictPreview,
...
} from "@/lib/product/release-readiness";
...
export function ReleaseReadinessCard(props: ReleaseReadinessCardProps) {
  ...
  const [latestDesktopVerification, setLatestDesktopVerification] =
    useState<DesktopVerificationHistoryEntry | null>(null);
  const [deploymentUrl, setDeploymentUrl] = useState("");
  const [screenshotUrl, setScreenshotUrl] = useState("");
  const [uploadEvidence, setUploadEvidence] =
    useState<SelfHostedUploadEvidencePacket | null>(null);
  const [profileReadinessEvidence, setProfileReadinessEvidence] =
    useState<SelfHostedUploadProfileReadinessEvidencePacket | null>(null);
  const [releaseImportMessage, setReleaseImportMessage] = useState("");
  const [desktopImportMessage, setDesktopImportMessage] = useState("");
  const [pendingReleaseImport, setPendingReleaseImport] =
    useState<PendingReleaseImport | null>(null);
  const [releaseEvidenceHistory, setReleaseEvidenceHistory] = useState<
    ReleaseEvidenceHistoryEntry[]
  >([]);
  const [releaseEvidenceHistoryFilter, setReleaseEvidenceHistoryFilter] =
    useState<ReleaseEvidenceHistoryFilter>("all");
  ...
  const evidenceSummary = useMemo(
    () => createReleaseEvidenceSummary(evidence),
    [evidence],
  );
  ...
  const freshDeploymentUrlCaptured = isReleaseEvidenceRequirementReady(
    evidenceSummary,
    "deployment-url",
  );
  const freshDeploymentScreenshotCaptured = isReleaseEvidenceRequirementReady(
    evidenceSummary,
    "deployment-screenshot",
  );
  const freshDesktopProofCaptured = isReleaseEvidenceRequirementReady(
    evidenceSummary,
    "desktop-proof",
  );
  const freshLatestDesktopVerification =
    latestDesktopVerification?.status === "ready" &&
    isReleaseEvidenceRequirementReady(
      latestDesktopEvidenceSummary,
      "desktop-proof",
    );
  const desktopVerified =
    props.desktopLaunchVerified ||
    freshDesktopProofCaptured ||
    freshLatestDesktopVerification;
  const deploymentPreflight = useMemo(
    () =>
      createDeploymentReleasePreflight({
        vercelLinked: props.vercelLinked,
        deploymentUrlCaptured:
          props.deploymentUrlCaptured || freshDeploymentUrlCaptured,
        deploymentScreenshotCaptured:
  ...
  const filteredReleaseEvidenceHistory = useMemo(
    () =>
      filterReleaseEvidenceHistory(
        releaseEvidenceHistory,
        releaseEvidenceHistoryFilter,
      ),
    [releaseEvidenceHistory, releaseEvidenceHistoryFilter],
  );
  const pinnedReleaseEvidence = useMemo(
    () =>
      releaseEvidenceHistory.find(
        (entry) => entry.id === pinnedReleaseEvidenceId,
      ) ??
      releaseEvidenceHistory[0] ??
      null,
  ...
  useEffect(() => {
    ...
    setUploadEvidence(
      createSelfHostedUploadEvidencePacket(loadSelfHostedUploadHistory()),
    );
    setProfileReadinessEvidence(
      createSelfHostedUploadProfileReadinessEvidencePacket(
        loadSelfHostedUploadProfileReadinessHistory(),
      ),
    );
    ...
    setEvidence(
      saveReleaseEvidence(
        isReleaseScreenshotUrl(proof)
          ...
          : {
              deploymentScreenshotUrl: "",
              deploymentScreenshotArtifact: proof,
            },
      ),
    );
  }
  ...
  function handleUseDesktopVerification() {
    ...
    setEvidence(
      saveReleaseEvidence(createReleaseDesktopProof(latestDesktopVerification)),
    );
    setDesktopImportMessage(
      `Using desktop check from ${formatEvidenceTime(latestDesktopVerification.checkedAt)}.`,
    );
  }

  async function handleImportDesktopEvidence(
    ...
    try {
      const entry = selectReadyDesktopVerificationEntry(
        JSON.parse(await file.text()),
      );

      if (!entry) {
        setDesktopImportMessage(
          "No ready desktop check was found in that evidence packet.",
        );
        return;
      }

      const restoredEntry =
        importDesktopVerificationHistoryEntry(entry).find(
          (historyEntry) => historyEntry.id === entry.id,
        ) ?? entry;
      setLatestDesktopVerification(restoredEntry);
      setEvidence(
        saveReleaseEvidence(createReleaseDesktopProof(restoredEntry)),
      );
      setDesktopImportMessage(
        `Imported desktop check from ${formatEvidenceTime(restoredEntry.checkedAt)}.`,
      );
    } catch {
  ...
  async function handleImportReleaseEvidence(
    event: ChangeEvent<HTMLInputElement>,
  ) {
    const file = event.target.files?.[0];
    event.target.value = "";
    ...
    try {
      ...
      if (pendingImport.preview.status === "conflict") {
        ...
        setReleaseImportMessage(
          "Review the release proof import conflicts before overwriting local evidence.",
        );
        return;
      }
  ...
  function createPendingReleaseImport(
    parsed: unknown,
  ): PendingReleaseImport | null {
    const importedEvidence = selectReleaseEvidenceFromPacket(parsed);
    if (!importedEvidence) return null;

    const importedDesktopVerification =
      selectReadyDesktopVerificationEntry(parsed);
    const importedUploadEvidence = selectUploadEvidenceFromPacket(parsed);
    const importedProfileReadinessEvidence =
      selectProfileReadinessEvidenceFromPacket(parsed);
  ...
  function applyReleaseEvidenceImport(pendingImport: PendingReleaseImport) {
    const nextEvidence = saveReleaseEvidence({
      ...
      ...(pendingImport.desktopVerification
        ? createReleaseDesktopProof(pendingImport.desktopVerification)
        : {}),
    });
    ...
    if (importedDesktopVerification) {
      const restoredDesktopVerification =
        importDesktopVerificationHistoryEntry(importedDesktopVerification).find(
          (historyEntry) => historyEntry.id === importedDesktopVerification.id,
        ) ?? importedDesktopVerification;
      setLatestDesktopVerification(restoredDesktopVerification);
      importDetails.push(
        `desktop check from ${formatEvidenceTime(restoredDesktopVerification.checkedAt)}`,
      );
    }

    if (pendingImport.uploadEvidence) {
      const nextUploadHistory = importSelfHostedUploadEvidencePacket(
        pendingImport.uploadEvidence,
      );
      const nextUploadEvidence =
        createSelfHostedUploadEvidencePacket(nextUploadHistory);
    ...
    if (pendingImport.profileReadinessEvidence) {
      const nextProfileReadinessHistory =
        importSelfHostedUploadProfileReadinessEvidencePacket(
          pendingImport.profileReadinessEvidence,
        );
      const nextProfileReadinessEvidence =
        createSelfHostedUploadProfileReadinessEvidencePacket(
          nextProfileReadinessHistory,
        );
      setProfileReadinessEvidence(nextProfileReadinessEvidence);
      importDetails.push(
        `${nextProfileReadinessEvidence.reportCount} profile readiness checks`,
      );
    }
    ...
    setReleaseImportMessage(
      importDetails.length > 0
        ? `Imported release proof packet with ${importDetails.join(" and ")}.`
        : "Imported release proof packet.",
    );
  }

  function handleSaveReleaseEvidenceSnapshot() {
    const nextHistory = saveCurrentReleaseEvidenceHistoryEntry(
      report,
      evidence,
      {
        desktopVerification: latestDesktopVerification,
  ...
  function handleReverifyReleaseEvidence(entry: ReleaseEvidenceHistoryEntry) {
    ...
    setPinnedReleaseEvidenceId(loadPinnedReleaseEvidenceHistoryId());
    setReleaseImportMessage(
      `Re-verified ${releaseEvidenceHistoryLabel(nextHistory[0] ?? entry).toLowerCase()} release proof.`,
    );
  }
    ...
    <Card className="shadow-none">
      <CardHeader>
        <CardTitle className="flex items-center justify-between gap-3 text-lg">
          ...
          <Badge variant={releaseBadgeVariant(report.status)}>
            {report.score}/100
          </Badge>
        </CardTitle>
      </CardHeader>
        ...
        <div className="space-y-3 rounded-md border border-border p-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                {evidence.updatedAt
                  ? `Saved ${formatEvidenceTime(evidence.updatedAt)}`
                  : "No saved proof yet"}{" "}
                / {evidenceSummary.readyCount} of {evidenceSummary.total} ready
              </div>
            </div>
            <Button
              size="sm"
            ...
            {evidenceSummary.requirements.map((requirement) => (
              <div key={requirement.id} className="rounded-md bg-muted/40 p-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge
                    variant={releaseEvidenceBadgeVariant(requirement.status)}
                  >
                    {releaseEvidenceStatusLabel(requirement.status)}
                  </Badge>
                </div>
          ...
          <div className="rounded-md border border-border p-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <Badge
                variant={
                  releaseAudit.status === "ready" ? "default" : "secondary"
                }
              >
                {releaseAudit.status === "ready" ? "Ready" : "Blocked"}
              </Badge>
            </div>
            {releaseAudit.status !== "ready" ? (
          ...
          <Input
            ...
            aria-label="Import release evidence packet"
            onChange={handleImportReleaseEvidence}
          />
          ...
          <div className="rounded-md border border-border p-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={handleSaveReleaseEvidenceSnapshot}
                aria-label="Save release evidence snapshot"
              >
                <Download className="size-3.5" />
                Save snapshot
              </Button>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {(["all", "ready", "draft", "stale"] as const).map((filter) => (
                <Button
                  ...
                  size="sm"
                  variant={
                    releaseEvidenceHistoryFilter === filter
                      ? "default"
                      : "outline"
                  }
                  ...
                  <div key={entry.id} className="rounded-md bg-muted/40 p-2">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <Badge variant={releaseHistoryBadgeVariant(entry)}>
                            {releaseEvidenceHistoryLabel(entry)}
                          </Badge>
                          {pinnedReleaseEvidenceId === entry.id ? (
                            <Badge variant="outline">Pinned</Badge>
                          ) : null}
                          <span className="text-xs text-muted-foreground">
                            {formatEvidenceTime(entry.savedAt)}
                          </span>
                        </div>
                        <div className="mt-1 truncate text-xs text-muted-foreground">
                          Release {entry.releaseScore}/100 / proof{" "}
                    ...
                    0 ? (
                      <div className="mt-2 text-xs text-muted-foreground">
                        {entry.missingCount} missing / {entry.staleCount} stale
                        / {entry.blockerCount} blocked / {entry.warningCount}{" "}
                        warnings
                      </div>
                    ) : null}
                  </div>
          ...
          <div className="rounded-md border border-border p-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <Badge
                variant={
                  uploadEvidence && uploadEvidence.verifiedCount > 0
                    ? "default"
                    : "outline"
                }
              >
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Upload evidence is included in the release packet so self-hosted
              media handoff proof travels with deployment and desktop proof.
            </p>
          </div>
          <div className="rounded-md border border-border p-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {profileReadinessEvidence &&
                  profileReadinessEvidence.reportCount > 0
              ...
              <Badge
                variant={
                  profileReadinessEvidence &&
                  profileReadinessEvidence.readyCount > 0
                    ? "default"
                    : "outline"
                }
              >
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Profile readiness evidence is included in the release packet so
              storage setup validation travels with upload handoff proof.
            </p>
          </div>
          <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
            ...
            <Button
              ...
              disabled={!canSaveDeploymentUrl}
              aria-label="Save deployed app URL"
            >
            ...
            <Button
              type="button"
              variant="outline"
              onClick={handleSaveScreenshotProof}
              disabled={!canSaveScreenshot}
              aria-label="Save deployed screenshot proof"
            >
              <CheckCircle2 className="size-3.5" />
              Save proof
            </Button>
          ...
          {localDeploymentProof ? (
            <div className="rounded-md bg-muted/40 p-2 text-xs text-muted-foreground">
              Deployment evidence saved: URL and screenshot are present.
          ...
          {savedScreenshotProof ? (
            <div className="rounded-md bg-muted/40 p-2 text-xs text-muted-foreground">
              ...
              {evidence.deploymentScreenshotUrl ? (
                <a
                  className="font-medium text-foreground underline-offset-4 hover:underline"
                  href={evidence.deploymentScreenshotUrl}
                  rel="noreferrer"
                  target="_blank"
                >
                  {evidence.deploymentScreenshotUrl}
                ...
                <span className="font-medium text-foreground">
                  {evidence.deploymentScreenshotArtifact}
                </span>
              )}
            </div>
          ) : null}
            ...
            <div className="rounded-md border border-border p-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {deploymentPreflight.summary}
                  </div>
                </div>
                <Badge
                  variant={releaseEvidenceBadgeVariant(
                    deploymentPreflight.status,
                  )}
                >
                  {releaseEvidenceStatusLabel(deploymentPreflight.status)}
                </Badge>
              </div>
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {deploymentPreflight.steps.map((step) => (
                  <div key={step.id} className="rounded-md bg-muted/40 p-2">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Badge variant={releaseEvidenceBadgeVariant(step.status)}>
                        {releaseEvidenceStatusLabel(step.status)}
                      </Badge>
                    </div>
                    <p className="mt-1 text-xs text-muted-foreground">
                      {step.detail}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Input
              ...
              onChange={handleImportDesktopEvidence}
            />
            <Button
              type="button"
              size="sm"
              ...
              aria-label="Clear release proof"
            >
              <Trash2 className="size-3.5" />
              Clear proof
            </Button>
          </div>
          {desktopImportMessage ? (
            <div className="rounded-md bg-muted/40 p-2 text-xs text-muted-foreground">
              {desktopImportMessage}
            </div>
          ) : null}
          {latestDesktopVerification ? (
            <div className="rounded-md bg-muted/40 p-2 text-xs text-muted-foreground">
              ...
              on {formatEvidenceTime(latestDesktopVerification.checkedAt)}
            </div>
          ) : null}
          {hasReleaseDesktopProof(evidence) ? (
            <div className="rounded-md bg-muted/40 p-2 text-xs text-muted-foreground">
              Desktop evidence saved: {evidence.desktopVerificationStepCount}{" "}
              checks from{" "}
              {formatEvidenceTime(
                evidence.desktopVerificationCheckedAt ??
                  evidence.updatedAt ??
                  Date.now(),
              )}
              . Freshness is shown above.
          ...
          {report.gates.map((gate) => (
            <div key={gate.id} className="rounded-md border border-border p-3">
              <div className="flex items-center justify-between gap-3">
                ...
                <Badge variant={releaseBadgeVariant(gate.status)}>
                  {releaseStatusLabel(gate.status)}
                </Badge>
              </div>
              <p className="mt-2 text-xs text-muted-foreground">
                {gate.detail}
              ...
              {gate.status !== "ready" ? (
                <p className="mt-2 text-xs">{gate.nextStep}</p>
              ) : null}
            </div>
          ))}
        </div>
...
function isReleaseEvidencePacket(
  value: unknown,
): value is Parameters<typeof saveReleaseEvidenceHistoryEntry>[0] {
