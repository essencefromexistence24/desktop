...
export function ReleaseOperationsHistoryCard() {
  ...
  const filteredHistory = useMemo(
    () => filterReleaseOperationsHistory(history, filter),
    [filter, history],
  );
  ...
  function saveSnapshot() {
    ...
    setHistory(entries);
    setMessage(
      `Saved ${releaseOperationsStatusLabel(nextPacket.status).toLowerCase()} release operations snapshot.`,
    );
  }
  ...
  function exportPacket() {
    ...
    setPacket(nextPacket);
    setMessage(
      downloadReleaseOperationsPacket(nextPacket)
        ? "Release operations packet exported."
        : "Release operations export is unavailable in this runtime.",
    );
  }
    ...
    <Card className="shadow-none">
      <CardHeader>
        <CardTitle className="flex flex-wrap items-center justify-between gap-3 text-lg">
          ...
          <span className="flex flex-wrap items-center gap-2">
            <Badge variant={operationsBadgeVariant(packet?.status ?? "draft")}>
              {releaseOperationsStatusLabel(packet?.status ?? "draft")}
            </Badge>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <OperationMetric
            label="Release"
            value={packet ? `${packet.releaseEvidence.score}/100` : "Draft"}
          />
          <OperationMetric
            label="Deployment"
            value={packet?.deploymentProof.ready ? "Ready" : "Draft"}
          />
          <OperationMetric
            label="Maintenance"
            value={
              packet
                ? releaseOperationsStatusLabel(
                    packet.maintenanceEvidence.status,
                  )
                : "Draft"
            }
          />
          <OperationMetric
            ...
            value={
              packet
                ? releaseOperationsStatusLabel(packet.reviewHandoff.status)
                : "Draft"
            }
          />
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            aria-label="Save release operations snapshot"
          >
          ...
          <Button
            ...
            aria-label="Export release operations packet"
          >
        ...
        <div className="flex flex-wrap gap-2">
          {(["all", "ready", "draft", "blocked"] as const).map((nextFilter) => (
            ...
            >
              {nextFilter === "all"
                ? "All"
                : releaseOperationsStatusLabel(nextFilter)}
            </Button>
          ))}
        </div>
        <div className="grid gap-2">
          {filteredHistory.length > 0 ? (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <Badge variant={operationsBadgeVariant(entry.status)}>
                      {releaseOperationsStatusLabel(entry.status)}
                    </Badge>
                    <span className="text-xs text-muted-foreground">
                      {formatOperationsTime(entry.savedAt)}
                    </span>
                  </div>
                  <Badge variant="outline">
                    Release {entry.releaseScore}/100
                  </Badge>
                </div>
                <div className="mt-1 truncate text-xs text-muted-foreground">
                  Deployment {entry.deploymentReady ? "ready" : "draft"} /
                  maintenance{" "}
                  {releaseOperationsStatusLabel(
            ...
            <div className="rounded-md bg-muted/40 p-2 text-xs text-muted-foreground">
              No release operations snapshots match this filter.
            </div>
          )}
        </div>
      </CardContent>
...
function createCurrentReleaseOperationsPacket() {
  ...
  return createReleaseOperationsPacket({
    ...
    releaseHistoryEntry:
      selectPinnedReleaseEvidenceHistoryEntry(releaseHistory),
