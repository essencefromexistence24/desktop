...
export function HostedShareFreshnessWarning({
  report,
}: {
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge variant={freshnessBadgeVariant(item.status)}>
                {hostedShareFreshnessStatusLabel(item.status)}
              </Badge>
            </div>
...
function freshnessBadgeVariant(
  status: HostedShareFreshnessStatus | HostedShareFreshnessItemStatus,
) {
