...
import {
  getCloudWorkspaceAccess,
  mutateCloudWorkspaceAccess,
  type ServerWorkspaceAccessSummary,
} from "@/lib/projects/workspace-access-client";
import type {
  ServerFolderVisibility,
  ServerProjectFolderAccess,
  ServerWorkspaceRole,
...
export function CloudWorkspaceAccessPanel({
  projectId,
  disabled,
}: CloudWorkspaceAccessPanelProps) {
  const [access, setAccess] = useState<ServerWorkspaceAccessSummary | null>(
    null,
  );
  ...
  const [permissionRole, setPermissionRole] =
    useState<ServerWorkspaceRole>("viewer");
  const [folderName, setFolderName] = useState("");
  const [folderVisibility, setFolderVisibilityChoice] =
    useState<ServerFolderVisibility>("workspace");

  const permissions = useMemo(
    () => createWorkspacePermissionSet(access?.activeRole ?? "viewer"),
    [access?.activeRole],
  );
  ...
  useEffect(() => {
    ...
    void refresh()
      .catch(() => {
        setAccess(null);
        setMessage(
          "Signed-in workspace access appears after this project is synced.",
        );
      })
      .finally(() => setIsLoading(false));
  }, [refresh]);

  async function runAccessAction(
    label: string,
    action: () => Promise<ServerWorkspaceAccessSummary>,
  ) {
    ...
    try {
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Workspace access action failed.",
      );
    } finally {
  ...
  async function revokeInvitation(invitationId: string) {
    await runAccessAction(`revoke-${invitationId}`, () =>
      mutateCloudWorkspaceAccess(projectId, {
  ...
  function emailInvitation(
    invitation: ServerWorkspaceAccessSummary["invitations"][number],
  ) {
  ...
  async function removeProjectPermission(permissionId: string) {
    await runAccessAction(`remove-${permissionId}`, () =>
      mutateCloudWorkspaceAccess(projectId, {
        action: "remove-project-permission",
        permissionId,
      }),
    );
  }
  ...
  async function setFolderAccess(
    ...
    await runAccessAction("folder-access", () =>
      mutateCloudWorkspaceAccess(projectId, {
        action: "set-project-folder-access",
        folderId,
        folderAccess,
      }),
    );
  ...
  async function updateFolderVisibility(
    ...
    await runAccessAction(`visibility-${folderId}`, () =>
      mutateCloudWorkspaceAccess(projectId, {
        action: "set-folder-visibility",
        folderId,
        visibility,
      }),
    );
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Server-enforced access for synced projects.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant={access ? "default" : "secondary"}>
            {access ? workspaceRoleLabels[access.activeRole] : "Offline"}
          </Badge>
          <Button
            size="sm"
            variant="ghost"
            onClick={() => void refresh()}
            disabled={disabled || isLoading || Boolean(pendingAction)}
          >
              ...
              rows={pendingInvitations.map((invitation) => ({
                ...
                actions: [
                  ...
                  },
                  {
                    ...
                    onAction: () =>
                      void copyInvitationLink(invitation.acceptPath),
                  },
                  {
                    label: "Revoke",
          ...
          <section className="space-y-2 rounded-md border border-border p-3 lg:col-span-2">
            ...
            <div className="grid gap-2 sm:grid-cols-[1fr_140px_auto]">
              <Input
                ...
                placeholder="Private cut reviews"
                disabled={!canAct || !permissions.canManageFolders}
              />
              <Select
                value={folderVisibility}
                onValueChange={(value) =>
                  setFolderVisibilityChoice(value as ServerFolderVisibility)
                }
                disabled={!canAct || !permissions.canManageFolders}
              >
                <SelectTrigger>
              ...
              <Button
                ...
                disabled={
                  !folderName.trim() || !canAct || !permissions.canManageFolders
                }
              >
                Create
              </Button>
            ...
            <div className="grid gap-2 md:grid-cols-[1fr_180px]">
              ...
              <Select
                value={access.folderAccess}
                onValueChange={(value) =>
                  setFolderAccess(
                    access.folderId,
                    value as ServerProjectFolderAccess,
                  )
                }
                disabled={
                  !access.folderId ||
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateFolderVisibility(
                        folder.id,
                        value as ServerFolderVisibility,
                      )
                    }
                    disabled={!canAct || !permissions.canManageFolders}
                  >
...
function RoleSelect({
  ...
  value: ServerWorkspaceRole;
  onChange: (value: ServerWorkspaceRole) => void;
  disabled?: boolean;
}) {
  return (
    <Select
      value={value}
      onValueChange={(nextValue) => onChange(nextValue as ServerWorkspaceRole)}
      disabled={disabled}
    >
      <SelectTrigger>
        <SelectValue />
...
function AccessList({
  ...
  rows: Array<{
    ...
    title: string;
    meta: string;
    actions: Array<{ label: string; icon?: ReactNode; onAction: () => void }>;
  }>;
  empty: string;
  disabled?: boolean;
}) {
