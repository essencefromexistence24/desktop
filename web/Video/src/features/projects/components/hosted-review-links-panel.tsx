...
type HostedReviewLinkSort =
  "newest" | "oldest" | "expires-soon" | "proof-attention";
...
const permissionOptions: Array<{
  value: HostedReviewLinkPermission;
  label: string;
}> = [
...
export function HostedReviewLinksPanel({
  projectId,
  exportName,
  canManage,
  disabled,
}: HostedReviewLinksPanelProps) {
  ...
  const [freshnessContext, setFreshnessContext] =
    useState<HostedShareFreshnessContext>({
  ...
  const projectLinks = useMemo(
    () => links.filter((link) => link.projectId === projectId),
    [links, projectId],
  );
  ...
  const refresh = useCallback(async () => {
    const [nextLinks, projectReviews] = await Promise.all([
      listHostedProjectReviewLinks(),
      listProjectExportReviews(projectId),
    ]);
    setLinks(nextLinks);
  ...
  useEffect(() => {
    ...
    void refresh()
      .catch(() =>
        setMessage(
          "Hosted links are available after sign-in and project sync.",
        ),
      )
  ...
  async function updatePermission(
    ...
    await runLinkAction(link.id, async () => {
      replaceLink(
        await updateHostedProjectReviewLink({ id: link.id, permission }),
      );
  ...
  async function renewLink(link: HostedReviewLinkSummary) {
    await runLinkAction(link.id, async () => {
      replaceLink(
        await updateHostedProjectReviewLink({
          id: link.id,
          enabled: true,
          expiresInDays: 30,
        }),
  ...
  async function revokeLink(link: HostedReviewLinkSummary) {
    await runLinkAction(link.id, async () => {
      replaceLink(
        await updateHostedProjectReviewLink({ id: link.id, enabled: false }),
      );
  ...
  async function copyLink(
    url: string,
    successMessage = "Hosted review link copied.",
  ) {
    try {
  ...
  async function runLinkAction(id: string, action: () => Promise<void>) {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Hosted link action failed.",
      );
    } finally {
      setPendingId(null);
    }
  ...
  function replaceLink(nextLink: HostedReviewLinkSummary) {
    setLinks((current) =>
      current.map((link) => (link.id === nextLink.id ? nextLink : link)),
    );
  }
  ...
  return (
    <div className="space-y-3 rounded-md border border-border p-3 text-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Manage public review access for synced project metadata.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            disabled={disabled || isLoading || Boolean(pendingId)}
          >
      ...
      <div className="grid gap-2 md:grid-cols-[1fr_160px_160px]">
        ...
        <Select
          value={sortMode}
          onValueChange={(value) => setSortMode(value as HostedReviewLinkSort)}
        >
        ...
        <Select
          value={filterMode}
          onValueChange={(value) =>
            setFilterMode(value as HostedReviewLinkFilter)
          }
        >
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
            ...
            >
              ...
              <div className="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
                <Select
                  ...
                  onValueChange={(value) =>
                    void updatePermission(
                      link,
                      value as HostedReviewLinkPermission,
                    )
                  }
                ...
                <Button
                  ...
                  disabled={!canAct || pendingId === link.id || !link.enabled}
                >
          ...
          {visibleLinks.length === 0 ? (
            <div className="rounded-md border border-dashed border-border p-4 text-muted-foreground">
              No hosted links match these filters.
            </div>
          ) : null}
        </div>
      ) : (
        <div className="rounded-md border border-dashed border-border p-4 text-muted-foreground">
          No hosted links for this project yet.
        </div>
      )}
    </div>
  );
...
function matchesFilter(
  row: HostedReviewLinkRow,
  filter: HostedReviewLinkFilter,
) {
  if (filter === "active") return row.link.enabled && !row.link.expired;
  if (filter === "attention")
    return (
      !row.link.enabled || row.link.expired || row.report.status !== "ready"
    );
...
function compareRows(
  first: HostedReviewLinkRow,
  second: HostedReviewLinkRow,
  sort: HostedReviewLinkSort,
) {
  if (sort === "oldest")
    return first.link.createdAt.localeCompare(second.link.createdAt);
  if (sort === "expires-soon")
    return first.link.expiresAt.localeCompare(second.link.expiresAt);
  if (sort === "proof-attention") {
    const proofRank =
      freshnessRank(second.report.status) - freshnessRank(first.report.status);
...
function createFreshnessContext(
  projectId: string,
  projectReviews: ExportReviewPackage[],
): HostedShareFreshnessContext {
  const releaseEvidence = loadReleaseEvidence();
  const latestDesktopEvidence = loadDesktopVerificationHistory()[0] ?? null;
  const proofBundleHistory = loadExportProofBundleHistory().filter(
    (entry) => entry.bundle.projectId === projectId,
  );
  ...
  return {
    ...
    reviewProofUpdatedAt: newestEvidenceTimestamp([
      ...
      ...proofBundleHistory.flatMap((entry) => [
        entry.importedAt,
        entry.bundle.generatedAt,
      ]),
    ]),
  };
}
