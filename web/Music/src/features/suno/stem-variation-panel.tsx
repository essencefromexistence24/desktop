...
import type {
  StemVariationBusyState,
  StemVariationJobView,
} from "@/features/ai/use-stem-variation-job";

type StemVariationPanelProps = {
  busy: StemVariationBusyState;
  job?: StemVariationJobView;
  onRefresh: () => Promise<unknown>;
  onSave: () => Promise<unknown>;
};

export function StemVariationPanel({
  busy,
  job,
  onRefresh,
  onSave,
}: StemVariationPanelProps) {
  ...
  return (
    <Card className="border-white/10 bg-white/[0.04]">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              title={actionGuard.title("Save variation")}
              onClick={() => {
                void onSave();
              }}
            >
      ...
      <CardContent className="space-y-3">
        <div>
          <p className="text-sm font-medium">{job.sourceStemTitle}</p>
        ...
        {job.audio?.available && job.audio.audioUrl ? (
          ...
          <p className="text-sm text-muted-foreground">
            Waiting for the configured provider to return a real stem variation.
          </p>
        )}
      </CardContent>
    </Card>
