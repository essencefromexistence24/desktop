...
import type {
  RemasterBusyState,
  RemasterJobView,
} from "@/features/ai/use-remaster-job";

type RemasterPanelProps = {
  busy: RemasterBusyState;
  job?: RemasterJobView;
  onRefresh: () => Promise<unknown>;
  onSave: () => Promise<unknown>;
};

export function RemasterPanel({
  busy,
  job,
  onRefresh,
  onSave,
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              title={actionGuard.title("Save remaster")}
              onClick={() => {
                void onSave();
              }}
            >
      ...
      <CardContent className="space-y-3">
        ...
        {job.audio?.available && job.audio.audioUrl ? (
          ...
          <p className="text-sm text-muted-foreground">
            Waiting for the configured provider to return a real remastered file.
          </p>
        )}
      </CardContent>
    </Card>
