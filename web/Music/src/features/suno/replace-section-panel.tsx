...
import { RefreshCw, Save, Scissors } from "lucide-react";
...
import type {
  ReplaceSectionBusyState,
  ReplaceSectionJobView,
} from "@/features/ai/use-replace-section-job";

type ReplaceSectionPanelProps = {
  busy: ReplaceSectionBusyState;
  job?: ReplaceSectionJobView;
      ...
      <CardHeader>
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <CardTitle className="flex items-center gap-2 text-base">
            <Scissors className="size-4 text-emerald-200" />
            Replace section
          </CardTitle>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              title={actionGuard.title("Save edit")}
              onClick={() => {
                void onSave();
              }}
            >
      ...
      <CardContent className="space-y-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {job.id} / {job.mode} / {formatDuration(job.region.startMs)} -{" "}
            {formatDuration(job.region.endMs)}
          </p>
        ...
        {job.audio?.available && job.audio.audioUrl ? (
          ...
          <p className="text-sm text-muted-foreground">
            Waiting for the configured provider to return a real section edit.
          </p>
        )}
      </CardContent>
    </Card>
