...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    if (!source) {
      return jsonError("Cleaned audio is not available for this job.", 404);
    }
    ...
    if (!isSafeRemoteAudioUrl(source.value)) {
      return jsonError("Cleaned audio URL is not supported.", 422);
    }
    ...
    try {
      ...
      if (!providerResponse.ok || !providerResponse.body) {
        return jsonError("Remove FX provider result could not be fetched.", 502);
      }
...
function dataUrlResponse(source: AudioSource) {
  ...
  if (!match) {
    return jsonError("Inline cleaned audio is malformed.", 422);
  }
...
function audioHeaders(mediaType: string) {
  return {
    ...
    "Content-Disposition": 'inline; filename="essence-remove-fx"',
