...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    const source = parseVocalJobInput(job.input);
...
function getAudioSummary(
  ...
  return {
    available,
    audioUrl: available ? `/api/ai/vocal-jobs/${jobId}/audio` : "",
    mediaType:
      generation?.contentType || asset?.mediaType || outputAudio.mediaType || "",
    title: asset?.title || outputAudio.title || "Generated vocals",
  };
}
...
function parseVocalJobInput(input: unknown) {
  if (!input || typeof input !== "object") {
    return {
      ...
      voiceProfile: { name: "Voice profile" },
    };
  }
