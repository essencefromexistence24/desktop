 ...
 *
 * Always returns false: login is disabled for this build, so the dashboard and
 * management surface are usable without authenticating. The authz policy still
 * enforces its own gates (loopback-only routes, always-protected destructive
 * endpoints, API-key scopes) independently of this switch.
 */
