...
export default function Sidebar({
  ...
  const getIconStyle = (active: boolean): React.CSSProperties => ({
  ...
  const renderNavLink = (item) => {
    ...
    const className = cn(
      "flex items-center gap-3.5 rounded-lg transition-all group min-w-0 w-full max-w-full overflow-hidden",
    ...
    const iconClassName = cn(
      "material-symbols-outlined text-[18px] shrink-0 flex items-center justify-center",
      active && "fill-1",
    );
    const content = (
      <>
        <span className={iconClassName} style={getIconStyle(active)}>
          {item.icon}
        </span>
        {!collapsed && (
  ...
  return (
    <>
      <aside
        ref={sidebarRef}
        className={cn(
          "flex h-full min-h-0 flex-col border-r border-black/5 bg-sidebar transition-all duration-300 ease-in-out dark:border-white/5 overflow-hidden",
      ...
      >
        ...
        <div
          className={cn(
            ...
            collapsed ? "flex-col px-3 pb-3" : "px-4 pb-4 gap-2",
          )}
        >
          <Link
            href="/home"
            className={cn("flex items-center", collapsed ? "justify-center" : "gap-2.5 flex-1")}
          >
            <div className="flex items-center justify-center size-8 shrink-0">
              {customLogo ? (
                ...
                <DxLogo size={22} className="text-white" />
              )}
            </div>
            {!collapsed && (
              <div className="flex flex-col min-w-0">
                ...
                <span className="text-[10px] text-text-muted">v{APP_CONFIG.displayVersion}</span>
              </div>
            )}
          </Link>
          {onToggleCollapse && (
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden custom-scrollbar">
          <nav
            ...
            className={cn("min-h-0 w-full", collapsed ? "px-2 space-y-0.5" : "px-3 py-1")}
          >
            ...
            {displaySections.map((section, idx) => {
              ...
              return (
                <div key={section.id} className={cn(isFirst ? "space-y-0.5" : "mt-2", "min-w-0")}>
                  <div
                    className="flex items-center gap-1 px-4 py-1.5 rounded-md hover:bg-surface/30 transition-colors cursor-pointer group/header min-w-0"
                    onClick={() => toggleSection(sectionId)}
                    role="button"
                    aria-expanded={isExpanded}
                  >
                    <span className="flex-1 text-[10px] font-semibold text-text-muted/60 uppercase tracking-wider truncate group-hover/header:text-text-muted/90 transition-colors">
                      {section.title}
                    </span>
                      ...
                      {section.children.map((child: any) => {
                        if (child.type === "group") {
                          ...
                          return (
                            <div
                              key={child.id}
                              className={cn(separatorHidden ? "mt-0.5" : "mt-2", "overflow-hidden")}
                            >
                              {!separatorHidden && (
                                <div className="flex items-center gap-2 px-4 py-1 mb-1 min-w-0">
                                  <span className="text-[8px] font-semibold text-text-muted/40 uppercase tracking-widest truncate shrink min-w-0">
                                    {child.title}
                                  </span>
        ...
        >
          <button
            ...
            className={cn(
              ...
              "text-text-muted hover:text-text-main hover:bg-surface/60 border border-border hover:border-border",
