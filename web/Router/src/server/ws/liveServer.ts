...
/**
 * Whether anonymous (token-less) WebSocket connections are accepted.
 * Defaults to enabled so the dashboard works from any URL/port/host without a
 * login or API key. Set `LIVE_WS_ALLOW_ANONYMOUS=0` to re-enforce auth.
 */
function isAnonymousLiveWsAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env.LIVE_WS_ALLOW_ANONYMOUS?.trim().toLowerCase();
  if (!raw) return true;
...
async function authorizeConnection(request: import("http").IncomingMessage): Promise<WsAuthResult> {
  ...
  // Browser WebSocket clients cannot set custom Authorization headers. With no
  // token present, anonymous connections are accepted by default. When
  // LIVE_WS_ALLOW_ANONYMOUS=0, fall back to the dashboard session cookie
  // (same-origin proxy auth) before rejecting. Keep the check local to this
  // sidecar so it does not import Next.js-only modules.
  if (!token) {
    if (isAnonymousLiveWsAllowed()) {
 ...
 * LAN. Origin checks and token auth are off by default (any Origin / anonymous
 * connection accepted); set LIVE_WS_ALLOW_ANONYMOUS=0 to require a token or
...
export async function startLiveDashboardServer(
  ...
  if (!process.env.JWT_SECRET && !isAnonymousLiveWsAllowed()) {
    console.warn(
  ...
  wss.on("connection", async (ws, request) => {
    ...
    // Origin check — accepts any Origin (isOriginAllowed is open), kept so the
    // policy lives in one place (see liveServerAllowList.ts).
