...
/**
 * Origin check for dashboard mutations (non-GET browser requests).
 *
 * Always accepts: the dashboard can be served from any URL / port / host, so
 * the origin allow-list is disabled here too. Same-origin browser requests are
 * already unblocked, and this closes the last gap for proxies or direct
 * network access that present an unlisted Origin.
 */
export function validateBrowserMutationOrigin(_request: Request): BrowserMutationOriginVerdict {
  return { ok: true };
}
