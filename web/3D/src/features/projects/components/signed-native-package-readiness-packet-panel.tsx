...
import type {
  ...
  SignedNativePackageReadinessRow,
  SignedNativePackageReadinessStatus,
} from "@/features/projects/signed-native-package-readiness-packet";

function statusVariant(status: SignedNativePackageReadinessStatus) {
  if (status === "blocked") {
...
function StatusIcon({ status }: { status: SignedNativePackageReadinessStatus }) {
  if (status === "ready") {
...
function ReadinessRow({ row }: { row: SignedNativePackageReadinessRow }) {
  return (
    <TableRow>
...
export function SignedNativePackageReadinessPacketPanel({ report }: { report: SignedNativePackageReadinessPacketReport }) {
  return (
    <Card className="mt-4">
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              Signed native package readiness
            </CardTitle>
            <CardDescription>Artifact provenance, signing evidence, updater metadata, and release recommendation for native desktop package promotion.</CardDescription>
          </div>
      ...
      <CardContent className="grid gap-4">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          ...
          <SummaryTile detail="ready evidence" label="Ready" value={`${report.summary.readyCount}`} />
        ...
        <div className="rounded-md border bg-background p-3">
          <p className="text-sm font-medium">Package readiness action</p>
        ...
        <Table>
          ...
          <TableBody>{report.rows.map((row) => <ReadinessRow key={row.id} row={row} />)}</TableBody>
        </Table>
      </CardContent>
    </Card>
  );
