...
import type {
  ...
  BoardReleaseArchiveStewardshipExecutivePacketRow,
  BoardReleaseArchiveStewardshipExecutivePacketStatus,
} from "@/features/projects/board-release-archive-stewardship-executive-packet";

function statusVariant(status: BoardReleaseArchiveStewardshipExecutivePacketStatus) {
  if (status === "blocked") {
...
function StatusIcon({ status }: { status: BoardReleaseArchiveStewardshipExecutivePacketStatus }) {
  if (status === "approved") {
...
function PacketRow({ row }: { row: BoardReleaseArchiveStewardshipExecutivePacketRow }) {
  return (
    <TableRow>
...
export function BoardReleaseArchiveStewardshipExecutivePacketPanel({
  report,
}: {
  report: BoardReleaseArchiveStewardshipExecutivePacketReport;
}) {
  return (
    <Card className="mt-4">
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              Archive stewardship executive packet
            </CardTitle>
            <CardDescription>Executive release recommendation across ownership rotation, evidence aging, exception budget, and continuity rehearsal evidence.</CardDescription>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Badge className="rounded-md" variant={report.summary.stewardshipScore < 80 ? "destructive" : "outline"}>
              {report.summary.stewardshipScore}/100 stewardship
            </Badge>
      ...
      <CardContent className="grid gap-4">
        ...
        <div className="rounded-md border bg-background p-3">
          ...
          <p className="mt-2 truncate font-mono text-xs text-muted-foreground">{report.summary.stewardshipPacketHash}</p>
        </div>

        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Stewardship area</TableHead>
