...
import type {
  ...
  BoardReleaseArchiveIntelligenceAutomationDigestRow,
  BoardReleaseArchiveIntelligenceAutomationDigestStatus,
} from "@/features/projects/board-release-archive-intelligence-automation-digest";

function statusVariant(status: BoardReleaseArchiveIntelligenceAutomationDigestStatus) {
  if (status === "blocked") {
...
function StatusIcon({ status }: { status: BoardReleaseArchiveIntelligenceAutomationDigestStatus }) {
  if (status === "ready") {
...
function DigestRow({ row }: { row: BoardReleaseArchiveIntelligenceAutomationDigestRow }) {
  return (
    <TableRow>
...
export function BoardReleaseArchiveIntelligenceAutomationDigestPanel({
  report,
}: {
  report: BoardReleaseArchiveIntelligenceAutomationDigestReport;
}) {
  return (
    <Card className="mt-4">
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              Archive intelligence automation digest
            </CardTitle>
            <CardDescription>Trend, approval, notification, and remediation follow-through summary for archive automation closeout.</CardDescription>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Badge className="rounded-md" variant={report.summary.digestScore < 70 ? "destructive" : "outline"}>
      ...
      <CardContent className="grid gap-4">
        <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
          ...
          <SummaryTile detail="open recommendations" label="Follow-through" value={`${report.summary.followThroughCount}`} />
          <SummaryTile detail="workspace" label="Digest" value={report.workspaceId} />
        </div>

        <div className="rounded-md border bg-background p-3">
          <p className="text-sm font-medium">Digest next action</p>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Digest area</TableHead>
