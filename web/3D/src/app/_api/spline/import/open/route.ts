...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(await openSplineProjectFromInput(payload, config), {
      headers: {
        "Cache-Control": "no-store",
      },
    });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Spline project import could not be opened." }, { status: 422 });
  }
}
