...
const workflow = createBoardAuditEvidenceAcceptanceWorkflow({
  acceptances: [
    {
...
assert.equal(workflow.summary.status, "blocked");
assert.equal(workflow.summary.acceptanceScore, 50);
assert.equal(workflow.summary.acceptedCount, 1);
assert.equal(workflow.summary.rejectedCount, 1);
assert.equal(workflow.summary.pendingCount, 0);
assert.equal(workflow.summary.auditTrailCount, 2);
assert.equal(workflow.rows.find((row) => row.taskId === "decision:decision-1")?.status, "accepted");
assert.equal(workflow.rows.find((row) => row.taskId === "evidence:packet:old")?.rejectionReason, "Missing refreshed packet evidence export.");
assert.match(workflow.auditTrail[0]?.summary ?? "", /accepted/);
assert.match(workflow.summary.nextAction, /Missing refreshed packet evidence export/);
assert.match(workflow.csvContent, /task_id,status,owner,verification_status,rejection_reason,acknowledged_at,next_action/);
assert.match(workflow.jsonContent, /"auditTrail"/);
assert.equal(workflow.csvFileName, "workspace-board-board-audit-evidence-acceptance-20260520.csv");

console.log("board audit evidence acceptance smoke passed");
