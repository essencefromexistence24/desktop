...
/**
 * Resolve the pnpm virtual-store location of a (possibly un-hoisted) package.
 *
 * pnpm keeps optional/platform packages (e.g. sqlite-vec-windows-x64) ONLY in
 * the virtual store at node_modules/.pnpm/<name>@<version>/node_modules/<name>
 * and does NOT hoist a symlink into the top-level node_modules, so the direct
 * node_modules/<name> path the sidecar entries use is absent. Fall back to the
 * first matching store entry so the standalone ships the real package.
 *
 * @param {string} projectRoot
 * @param {string} pkgName - e.g. "sqlite-vec-windows-x64" or "@scope/name"
 * @returns {string|null} the on-disk package dir, or null when not installed
 */
function resolvePnpmStoreSource(projectRoot, pkgName) {
  const storeDir = path.join(projectRoot, "node_modules", ".pnpm");
  let entries = [];
  try {
    entries = fsSync.readdirSync(storeDir);
  } catch {
    return null;
  }
  // pnpm store dirs are "<name>@<version>" (scoped: "@scope+name@<version>").
  const prefix = pkgName.startsWith("@")
    ? `${pkgName.slice(1).replace("/", "+")}@`
    : `${pkgName}@`;
  for (const entry of entries) {
    if (!entry.startsWith(prefix)) continue;
    const candidate = path.join(storeDir, entry, "node_modules", pkgName);
...
/**
 * Resolve the on-disk source for a sidecar entry.
 *
 * For bare node_modules packages, when the direct node_modules/<name> path is
 * absent (pnpm does not hoist platform-specific optional deps like
 * sqlite-vec-windows-x64 to the top level), fall back to the pnpm virtual
 * store. Returns null when nothing matches.
 *
 * @param {string} projectRoot
 * @param {string[]} srcSegments - entry.src path segments relative to projectRoot
 * @returns {string|null} the real source path, or null if absent
 */
function resolveEntrySource(projectRoot, srcSegments) {
  const direct = path.join(projectRoot, ...srcSegments);
  if (fsSync.existsSync(direct)) return direct;

  if (srcSegments.length === 2 && srcSegments[0] === "node_modules") {
    return resolvePnpmStoreSource(projectRoot, srcSegments[1]);
  }
  return null;
}

/**
 * Remove a destination that would block a recursive dir copy: nft traces can
 * emit a module as a single file or as a symlink into the pnpm store, and
 * cpSync(dir -> symlink/file) fails with ERR_FS_CP_DIR_TO_NON_DIR. Real
 * directories are left in place (cpSync force:true handles them).
 * @param {string} dest
 */
function clearNonDirectoryDest(dest) {
  try {
    const stat = fsSync.lstatSync(dest);
    if (!stat.isDirectory()) {
      fsSync.rmSync(dest, { recursive: true, force: true });
    }
  } catch {
    // dest does not exist — nothing to clear
  }
}
...
async function syncNativeAssetsToDir(projectRoot, outDir, fsImpl, log) {
  ...
  for (const entry of NATIVE_ASSET_ENTRIES) {
    const sourcePath = resolveEntrySource(projectRoot, entry.src);
...
function copyNativeAssetsAndExtraModules(projectRoot, resolvedOutDir) {
  for (const asset of NATIVE_ASSET_ENTRIES) {
    const src = resolveEntrySource(projectRoot, asset.src);
    if (!src) continue;
    const dest = path.join(resolvedOutDir, ...asset.dest);
    // nft may have emitted the destination as a single-file trace or a pnpm-store
    // symlink (Windows junction style) — cpSync(dir -> non-dir) throws
    // ERR_FS_CP_DIR_TO_NON_DIR. Clear a non-directory dest before copying.
    clearNonDirectoryDest(dest);
  ...
  for (const mod of EXTRA_MODULE_ENTRIES) {
    const src = resolveEntrySource(projectRoot, mod.src);
    if (!src) continue;
...
export function materializeBundledSymlinks(nodeModulesDir) {
  ...
  const walk = (dir, depth = 0) => {
    ...
    const realKey = dir.replaceAll("\\", "/").toLowerCase();
    ...
    for (const name of names) {
      ...
      if (stat.isSymbolicLink()) {
        ...
        // Links will be replaced by real copies; skip walking the link itself.
        continue;
      }
      if (stat.isDirectory()) {
        // Only descend into real directories. pnpm's virtual store keeps a
        // nested node_modules per package (store@<hash>/node_modules) whose
        // dependencies are symlinks into the store — those must be materialized
        // too, so recurse rather than stopping at depth 1.
  ...
  for (const entryPath of entries) {
    ...
    // Windows pnpm symlinks can fail stat/realpath with EPERM even when the
    // target exists (the link is a file-type reparse point Node refuses to
    // follow). Fall back to readlinkSync + manual resolution.
    if (!realTarget) {
      try {
        const linkTarget = fsSync.readlinkSync(entryPath);
        if (linkTarget) {
    ...
    // Dangling link (e.g. an absolute path into the build machine that no
    // longer exists). Try a sibling real package with the name stripped of any
    // trailing `-<hex>` hash.
